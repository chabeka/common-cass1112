 ...
 * 
 * Un ByteSequence est une expression régulière representé sous forme d'objet. 
 * Contenu dans le fichier signature des fichiers.
 * 
 * <p/>
 * Il est composé d'une liste de {@link SubSequence}.
 * 
 */
public class ByteSequence extends SimpleElement {
   ...
   // private int searchDirection = 1;// could be -1 as default? check original
   // code to be really sure about this.
   ...
   public final void setReference(final String theRef) {
      ...
      // IndirectEOFOffsets are really anchored to the BOF (but we find the BOF
      // offset relative to the EOF).
    ...
    */
   public final void setIndirectOffsetLocation(
         final String indirectOffsetLocation) {
   ...
   private void prepareSequenceFragments() {
      ...
      // files:
      final int firstSequenceToMatch = reverseOrder ? subSequences.size() - 1
            : 0;
    ...
    */
   public final boolean isInvalidByteSequence() {
   ...
   private int getIndirectOffset(final ByteReader targetFile) {
      ...
      if (this.hasIndirectOffset) {
         ...
         final net.domesdaybook.reader.ByteReader reader = targetFile
               .getReader();
         ...
         if (bigEndian) {
            for (int byteIndex = offsetLength - 1; byteIndex > -1; byteIndex--) {
               final Byte fileByte = reader
                     .readByte(offsetLocation + byteIndex);
    ...
    *           the maximum number of bytes to scan from the start or the end of
    *           a file, or a negative number meaning a full file scan is
    *           possible.
    * @return boolean
    */
   public final boolean matches(final ByteReader targetFile,
      ...
      if (reverseOrder) {
         ...
         for (int subSequenceIndex = seq.length - 1; matchResult
               && subSequenceIndex >= 0; subSequenceIndex--) {
            final SubSequence subseq = seq[subSequenceIndex];
            final long currentFilePos = targetFile.getFileMarker();
            matchResult = subseq.findSequenceFromPosition(currentFilePos,
                  targetFile, maxBytesToScan);
         }
      } else {
         final long offset = getIndirectOffset(targetFile);
         targetFile.setFileMarker(offset);
         for (int subSequenceIndex = 0; matchResult
               && subSequenceIndex < seq.length; subSequenceIndex++) {
            final SubSequence subseq = seq[subSequenceIndex];
            final long currentFilePos = targetFile.getFileMarker();
            matchResult = subseq.findSequenceFromPosition(currentFilePos,
                  targetFile, maxBytesToScan);
   ...
   public final String toRegularExpression(final boolean prettyPrint) {
      ...
      for (int subSequenceIndex = 0; subSequenceIndex < numSequences; subSequenceIndex++) {
         ...
         final String subSeqExpression = subseq
               .toRegularExpression(prettyPrint);

         // Append any bounded gaps at the start (or end if going backwards) of
         // the sequence:
         ByteSequence.appendBoundedGapExpression(prettyPrint, reverseOrder,
               regularExpression, subSeqExpression, minSeqOffset,
               subSequenceIndex == 0 ? maxSeqOffset : -1);
      }
   ...
   public final static String bytesToString(final boolean prettyPrint,
         byte[] bytes) {
      // CHECKSTYLE:ON
      ...
      for (int byteIndex = 0; byteIndex < byteLength; byteIndex++) {
         ...
         if (prettyPrint && byteValue >= START_PRINTABLE_ASCII_CHARS
            ...
            final String formatString = prettyPrint ? inString ? "' %02x"
    ...
    */
   public static final String byteValueToString(final boolean prettyPrint,
         final int byteValue) {
