...
public class CassandraIdGenerator implements IdGenerator {
   ...
   */
  public CassandraIdGenerator(final Keyspace keyspace,
                              final CuratorFramework curatorClient, final String sequenceName,final JobClockSupport jobClockSupport) {
  ...
  public final long getNextId() {
    ...
    final ZookeeperMutex mutex = new ZookeeperMutex(curatorClient, "/sequences/"
        + sequenceName);
    try {
      ...
      final HColumn<String, Long> currentSequence = readCurrentSequence();

      // On écrit dans cassandra la valeur incrémentée
      final long newValue = incrementSequenceValue(currentSequence);
   ...
   */
  private long incrementSequenceValue(final HColumn<String, Long> currentSequence) {
    long currentValue;
