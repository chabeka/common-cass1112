...
/**
 * Test un traitement de modification de masse en passant deux fois le même document
 */
public class ModificationMasseDoublonIdGedTest {

   private static final Logger LOGGER = LoggerFactory.getLogger(ModificationMasseDoublonIdGedTest.class);
    ...
    * Le 1er document est OK
    * Le 2ème et le 3ème ont le même Id GED
    * On vérifie que seul le 1er document est modifié
    * 
    * @throws Exception
    */
   public void modificationDoublonIdGed() throws Exception {
      ...
      // Pour le doc 1 : on tente de modifier la dénomination
      ...
      // Pour le doc 2 : on tente de modifier le Siren, mais on utiliser le même id ged
      final ListeMetadonneeType modifiedMetas2 = new ListeMetadonneeType();
      SoapBuilder.addMeta(modifiedMetas2, "Siren", "test");
      ...
      try (final JobManager job = new JobManager(environnementGNT)) {
         ...
         assertThat(libErreur, containsString("La métadonnée UUID est en doublon"));

         final NonIntegratedDocumentType nonIntegratedDocument2 = resultat.getNonIntegratedDocuments().getNonIntegratedDocument().get(1);
         Assert.assertEquals(uuids.get(1), nonIntegratedDocument2.getObjetNumerique().getUUID());
         final String libErreur2 = nonIntegratedDocument2.getErreurs().getErreur().get(0).getLibelle();
         assertThat(libErreur2, containsString("La métadonnée UUID est en doublon"));
