...
/**
 * Test un traitement de modification de masse qui tente de modifier une méta inexistante
 */
public class ModificationMasseMetaInexistanteTest {

   private static final Logger LOGGER = LoggerFactory.getLogger(ModificationMasseMetaInexistanteTest.class);
    ...
    * Sur le 2ème document, on tente de modifier la méta NumeroBis qui n'exise pas
    * Sur le 3ème document, on tente de supprimer la méta NumeroBis qui n'exise pas
    * On vérifie que les documents 1 a bien été modifié
    * 
    * @throws Exception
    */
   public void metaInexistante() throws Exception {
      ...
      // Pour le doc 1 : on tente de modifier NumeroBis qui n'existe pas
      final ListeMetadonneeType modifiedMetas1 = new ListeMetadonneeType();
      SoapBuilder.addMeta(modifiedMetas1, "NumeroBis", "123456789");
      builder.addDocument(uuids.get(1), modifiedMetas1);

      // Pour le doc 2 : on tente de supprimer NumeroBis qui n'existe pas
      final ListeMetadonneeType modifiedMetas2 = new ListeMetadonneeType();
      SoapBuilder.addMeta(modifiedMetas2, "NumeroBis", "");
      ...
      try (final JobManager job = new JobManager(environnementGNT)) {
         ...
         assertThat(libErreur, containsString("La ou les métadonnées suivantes n'existent pas dans le référentiel des métadonnées : NumeroBis"));
         ...
         assertThat(libErreur2, containsString("La ou les métadonnées suivantes n'existent pas dans le référentiel des métadonnées : NumeroBis"));
