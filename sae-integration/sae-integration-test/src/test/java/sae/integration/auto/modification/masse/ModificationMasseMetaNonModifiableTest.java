...
/**
 * Test un traitement de modification de masse qui tente de modifier une méta non modifiable
 */
public class ModificationMasseMetaNonModifiableTest {

   private static final Logger LOGGER = LoggerFactory.getLogger(ModificationMasseMetaNonModifiableTest.class);
    ...
    * Sur le 2ème document, on tente de modifier une méta non modifiable : TypeHash
    * Sur le 3ème document, on tente de modifier des métas non archivables : CodeFonction et DureeConservation
    * On vérifie que les document 1 est bien modifié, mais pas les autres
    * 
    * @throws Exception
    */
   public void modificationMetaNonModifiable() throws Exception {
      ...
      SoapBuilder.addMeta(modifiedMetas1, "TypeHash", "SHA-256");
      builder.addDocument(uuids.get(1), modifiedMetas1);

      // Pour le doc 2 : on de modifier CodeFonction et DureeConservation, qui ne sont pas archivables
      final ListeMetadonneeType modifiedMetas2 = new ListeMetadonneeType();
      SoapBuilder.addMeta(modifiedMetas2, "CodeFonction", "3");
      SoapBuilder.addMeta(modifiedMetas2, "DureeConservation", "125");
      ...
      try (final JobManager job = new JobManager(environnementGNT)) {
         ...
         assertThat(libErreur, containsString("La ou les métadonnées suivantes ne sont pas modifiables : TypeHash"));
         ...
         assertThat(libErreur2, containsString("La ou les métadonnées suivantes ne sont pas modifiables"));
         assertThat(libErreur2, containsString("CodeFonction"));
         assertThat(libErreur2, containsString("DureeConservation"));
