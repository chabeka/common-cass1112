...
/**
 * Test un traitement de modification de masse qui tente de modifier un document gelé
 */
public class ModificationMasseDocGelTest {

   private static final Logger LOGGER = LoggerFactory.getLogger(ModificationMasseDocGelTest.class);


   @Test
   /**
    * On archive 3 documents en GNT. On gèle le 2ème
    * On crée un sommaire avec 3 documents à modifier.
    * On vérifie que les documents 1 et 3 sont bien modifiés, mais pas le 2ème
    * 
    * @throws Exception
    */
   public void documentGele() throws Exception {
      ...
      // Gel du 2ème document
      GelUtils.gelDocument(environnementGNT, uuids.get(1));
      ...
      // Pour le doc 1 : on tente de modifier le Siren
      ...
      try (final JobManager job = new JobManager(environnementGNT)) {
         ...
         assertThat(libErreur, containsString("est gelé et ne peut pas être traité"));

      }
      finally {
         LOGGER.info("Dégel du document");
         GelUtils.degelDocument(environnementGNT, uuids.get(1));
