...
/**
 * Test un traitement de modification de masse qui tente de supprimer une méta qui est obligatoire à l'archivage
 * donc non supprimable
 */
public class ModificationMasseSuppressionMetaObligatoireTest {

   private static final Logger LOGGER = LoggerFactory.getLogger(ModificationMasseSuppressionMetaObligatoireTest.class);
    ...
    * Sur le 2ème document, on tente de supprimer une méta qui est obligatoire
    * On vérifie que les documents 1 et 3 sont bien modifiés, mais pas le 2ème
    * 
    * @throws Exception
    */
   public void suppressionMetaObligatoire() throws Exception {
      ...
      // Pour le doc 1 : on tente de supprimer le Titre, qui est obligatoire
      final ListeMetadonneeType modifiedMetas1 = new ListeMetadonneeType();
      SoapBuilder.addMeta(modifiedMetas1, "Titre", "");
      ...
      try (final JobManager job = new JobManager(environnementGNT)) {
         ...
         assertThat(libErreur, containsString("La ou les métadonnées suivantes ne peuvent pas être vides : Titre"));
