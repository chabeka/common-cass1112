...
public class ModificationRattrapageTest {
  ...
  @Test
  /**
   * Rattrapage sur UR747 : Changement du code RND qui est resté en générique et qui bloque la suppression
   * pendant un ADDT de WATT
   */
  public void rattrapageUR747() throws Exception {
    final SaeServicePortType service = SaeServiceStubFactory
        .getServiceForRechercheDocumentaireGNT("http://hwi69progednatgntcot1boweb1.cer69.recouv/ged/services/SaeService/");

    final ListeMetadonneeType fixedMetadatas = new ListeMetadonneeType();
    SoapBuilder.addMeta(fixedMetadatas, "DomaineCotisant", "true");
    SoapBuilder.addMeta(fixedMetadatas, "CodeRND", "7.7.8.6.1");
    SoapBuilder.addMeta(fixedMetadatas, "CodeOrganismeProprietaire", "UR747");
    final String newCodeRND = "1.A.X.X.X";
    final RangeMetadonneeType varyingMetadata = SoapBuilder.buildRangeMetadata("DateArchivage", "20200103", "20200103");
    final boolean simulationMode = false;
    modifyOneMetaByIteration(service, simulationMode, fixedMetadatas, varyingMetadata, "CodeRND", newCodeRND);
  }
