...
import fr.urssaf.image.sae.integration.ihmweb.saeservice.modele.SaeServiceStub.ConsultationAffichableResponse;
...
@Service
public class ConsultationAffichableTestService {
   ...
   private ConsultationResultat appelWsOpConsultationAffichable(
         String urlServiceWeb, ViStyle viStyle, ViFormulaire viParams,
         ConsultationAffichableFormulaire formulaire, WsTestListener wsListener) {
      ...
      SaeServiceLogUtils.logAppelConsultationAffichable(log, formulaire);
      ...
      try {
         ...
         ConsultationAffichableResponse response = service
               .consultationAffichable(paramsService);

         // Transtypage de l'objet de la couche ws vers l'objet du modèle
         result = fromConsultationAffichableService(response);
         ...
         log
               .appendLogLn("Détails de la réponse obtenue de l'opération \"consultationAffichable\" :");
   ...
   /**
    * Test libre de l'appel à l'opération "consultationAffichable" du service
    * web SaeService.<br>
    * 
    * @param urlServiceWeb
    ...
    *           le formulaire
    */
   public final void appelWsOpConsultationAffichableTestLibre(
         String urlServiceWeb, ConsultationAffichableFormulaire formulaire) {

      appelWsOpConsultationAffichableTestLibre(urlServiceWeb, formulaire, null);
   ...
   public final void appelWsOpConsultationAffichableTestLibre(
      ...
      appelWsOpConsultationAffichable(urlServiceWeb, ViStyle.VI_OK, viParams,
            formulaire, testLibre);

   }

   /**
    * Test d'appel à l'opération "consultationAffichable" du service web
    * SaeService.<br>
    ...
    *           attendue
    */
   public final void appelWsOpConsultationAffichableSoapFault(
         String urlServiceWeb, ConsultationAffichableFormulaire formulaire,
         ViFormulaire viParams) {
   ...
   public final void appelWsOpConsultationSoapFault(String urlServiceWeb,
         ConsultationAffichableFormulaire formulaire, ViStyle viStyle,
      ...
      // Appel de la méthode "générique" de test
      appelWsOpConsultationAffichable(urlServiceWeb, viStyle, null, formulaire,
            testAuth);
   ...
   public final void appelWsOpConsultationSoapFault(String urlServiceWeb,
      ...
      // Appel de la méthode "générique" de test
      appelWsOpConsultationAffichable(urlServiceWeb, viStyle, viParams,
            formulaire, testAuth);

   }

   private void wsVerifieRetour(ConsultationAffichableFormulaire formulaire,
    ...
    *         s'est produite
    */
   public final ConsultationResultat appelWsOpConsultationAffichableReponseCorrecteAttendue(
         String urlServiceWeb, ConsultationAffichableFormulaire formulaire,
         String sha1attendu) {

      return appelWsOpConsultationAffichableReponseCorrecteAttendue(
            urlServiceWeb, formulaire, sha1attendu, null, null);
   ...
   public final ConsultationResultat appelWsOpConsultationAffichableReponseCorrecteAttendue(
      ...
      // Appel de la méthode "générique" de test
      ConsultationResultat response = appelWsOpConsultationAffichable(
      ...
      if (!TestStatusEnum.Echec.equals(resultatTest.getStatus())) {
         ...
         if (response == null) {
            ...
            ResultatTestLog log = resultatTest.getLog();
            log
                  .appendLogLn("On s'attendait à une réponse sans erreur du service web, or on a obtenu une réponse null");
   ...
   private ConsultationResultat fromConsultationAffichableService(
         ConsultationAffichableResponse response) {

      // Création de l'objet résultat
      ConsultationResultat result = new ConsultationResultat();

      // Extrait le contenu (DataHandler)
      DataHandler contenu = response.getConsultationAffichableResponse()
            .getContenu();
      result.setContenu(contenu);

      // Extrait les métadonnées
      ListeMetadonneeType metadonnees = response
