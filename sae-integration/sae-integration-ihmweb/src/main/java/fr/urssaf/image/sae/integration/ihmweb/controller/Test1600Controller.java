...
/**
 * 1600-Ged-Technique-OK-Suppression-Doc-Existant
 */
@Controller
@RequestMapping(value = "test1600")
public class Test1600Controller extends
   ...
   protected final TestFormulaireSuppressionCuRe getFormulairePourGet() {
      ...
      captUnit
            .setUrlEcde(getEcdeService()
                  .construitUrlEcde(
                        "SAE_INTEGRATION/20110822/Ged-Technique-1600-Ged-Technique-OK-Suppression-Doc/documents/doc1.PDF"));
      ...
      metadonnees.add("Denomination",
            "Test 1600-Ged-Technique-OK-Suppression-Doc-Existant");
      ...
      // formulaire de recherche
      RechercheFormulaire formRechercheApresSuppr = formulaire.getRechercheApresSuppr();
      formRechercheApresSuppr.getResultats().setStatus(TestStatusEnum.SansStatus);
      
      formRechercheApresSuppr.setCodeMetadonnees(new CodeMetadonneeList());
   ...
   protected final void doPost(TestFormulaireSuppressionCuRe formulaire) {
      ...
      if ("1".equals(etape)) {
         ...
         recherche(formulaire.getUrlServiceWeb(), formulaire
               .getRecherche(), formulaire.getViFormulaire());
      } else if ("3".equals(etape)) {
         suppression(formulaire.getUrlServiceWeb(), formulaire
               .getSuppression());
      } else if ("4".equals(etape)) {
         rechercheApresSuppression(formulaire.getUrlServiceWeb(), formulaire
               .getRechercheApresSuppr(), formulaire.getViFormulaire());
      }
   }
   
   private void etape1captureUnitaireAppelWs(TestFormulaireSuppressionCuRe formulaire) {
      ...
      if (!TestStatusEnum.Echec.equals(formulaire.getCaptureUnitaire().getResultats().getStatus())) {
         formulaire.getSuppression().setIdDocument(UUID.fromString(resultat.getIdArchivage()));
      }
   }
   ...
   private void verifieResultatN(int numeroResultatRecherche,
      ...
      valeursAttendues.add("Titre", "AUTRE COURRIER ENTRANT RELATIF A LA GESTION DES DONNEES ADMINISTRATIVES");
   ...
   private void suppression(String urlWebService,
      ...
      // Appel de la méthode de test
      getSuppressionTestService().appelWsOpSuppressionReponseAttendue(
            urlWebService, formulaire, null);
   }
   
   private void rechercheApresSuppression(String urlServiceWeb, RechercheFormulaire formulaire,
         ViFormulaire viParams) {
