...
public class ConvertFactory {
   ...
   */
  public final TypeDocument wsToTypeDocument(final RNDTypeDocument rndTypeDoc) {
    final TypeDocument typeDoc = new TypeDocument();

    // Code RND
    final String codeRnd = rndTypeDoc.get_reference();
    typeDoc.setCode(codeRnd);

    final String tabRnd[] = codeRnd.split("\\.");
    ...
    if (rndTypeDoc.get_dureeArchivage() > 0) {
      final int dureeConservation = rndTypeDoc.get_dureeArchivage() * NB_JOUR_PAR_AN;
  ...
  public final List<TypeDocument> wsToDocumentsType(
                                                    final RNDTypeDocument listeRndTypeDocs[]) {
    final List<TypeDocument> listeTypeDocs = new ArrayList<>();
    for (final RNDTypeDocument rndTypeDocument : listeRndTypeDocs) {
      final TypeDocument typeDoc = wsToTypeDocument(rndTypeDocument);
