...
import fr.urssaf.image.sae.rnd.dao.support.facade.SaeBddSupportFacade;
...
public class MajRndServiceImpl implements MajRndService {
  ...
  @Autowired
  private SaeBddSupportFacade saeBddSupportFacade;
  ...
  public final void lancer() throws MajRndException {
    ...
    try {
      versionRndSae = saeBddSupportFacade.getVersionRnd();
    }
    ...
    if (StringUtils.isBlank(versionRndSae.getVersionEnCours())
      ...
      try {
        saeBddSupportFacade.updateRnd(listeTypeDocs);
      }
      catch (final SaeBddRuntimeException e) {
        LOGGER
        .error(
      ...
      LOGGER.info("{} - Mise à jour des correspondances dans la BDD du SAE",
                  trcPrefix);
      try {
        saeBddSupportFacade.updateCorrespondances(listeCorrespondances, versionRndSae.getVersionEnCours());
      }
      catch (final SaeBddRuntimeException e) {
        LOGGER
        .error(
      ...
      try {
        saeBddSupportFacade.updateVersionRnd(versionRndSae);
      }
      catch (final SaeBddRuntimeException e) {
        LOGGER
        .error(
    ...
    if (CollectionUtils.isNotEmpty(listeCodesTemporaires)) {
      ...
      .info(
            ...
            trcPrefix);
      try {
        saeBddSupportFacade.updateRnd(listeCodesTemporaires);
      }
      catch (final SaeBddRuntimeException e) {
        LOGGER
        .error(
