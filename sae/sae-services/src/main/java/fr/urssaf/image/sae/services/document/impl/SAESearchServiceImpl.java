...
SAESearchService {
  ...
  public PaginatedUntypedDocuments searchPaginated(
    ...
    try {
      ...
      // Log la requête lucene et l'index utilisé si la requête dure plus de sae.duree.max.requete.soap secondes.
  ...
  private void checkMetadataDoublon(final List<String> codeLongFiltresEgal, final List<String> codeLongFiltresDifferent)
    ...
    for (final Map.Entry<String, Integer> entry : comptage.entrySet()) {
      if (comptage.get(entry.getKey()) > 1) {
