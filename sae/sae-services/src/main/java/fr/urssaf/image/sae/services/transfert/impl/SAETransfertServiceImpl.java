...
public class SAETransfertServiceImpl extends AbstractSAEServices implements SAETransfertService {
	...
	public final void transfertDocument(StorageDocument document) throws TransfertException {
		...
		try {
			...
			for (final StorageDocumentNote note : listeNotes) {
				try {
					storageTransfertService.addDocumentNote(document.getUuid(), note.getContenu(), note.getAuteur(),
			...
			try {
				...
				// GNS
				if (docAttache != null) {
					storageTransfertService.addDocumentAttachment(document.getUuid(), docAttache.getName(),
	...
	private void updateMetaDocumentForTransfert(final StorageDocument document)
		...
		final StorageMetadata dateArchivageGNT = new StorageMetadata(
				StorageTechnicalMetadatas.DATE_ARCHIVE_GNT.getShortCode(), dateArchivage);
	...
	public final StorageDocument updateMetaDocumentForTransfertMasse(final StorageDocument document,
			...
			for (final StorageMetadata meta : document.getMetadatas()) {
				for (final StorageMetadata meta2 : listeMeta) {
					if (meta.getShortCode().equals(meta2.getShortCode())
							&& StringUtils.isNotBlank(meta2.getValue().toString())) {
		...
		if (idTransfertMasseInterne == null || StringUtils.isEmpty(idTransfertMasseInterne)) {
			document.getMetadatas().add(new StorageMetadata(
					StorageTechnicalMetadatas.ID_TRANSFERT_MASSE_INTERNE.getShortCode(), idTraitementMasse.toString()));
		...
		try {
			if (StringUtils.isNotBlank(codeRnd)) {
				...
				final Date date = (Date) getValueMetaByCode(
						StorageTechnicalMetadatas.DATE_DEBUT_CONSERVATION.getShortCode(), metadataMasse);
	...
	public final StorageDocument controleDocumentTransfertMasse(final UUID idArchive,
		...
		try {
			...
			final StorageDocument documentGNS = transfertControlePlateforme(document, idArchive, isReprise,
					idTraitementMasse);
			...
			if (documentGNS != null) {
				...
				if (isReprise) {
					...
					if (ckeckIdTraitementExiste(documentGNS.getMetadatas(), idTraitementMasse)) {
						try {
							storageTransfertService.deleteStorageDocument(idArchive);
							LOG.info("{} - Reprise - Suppression du document {} de la GNS.", "transfertDoc",
									idArchive.toString());
			...
			if (document == null) {
				...
				if (documentGNS == null
					...
					if (getHashDocument(documentGNS) != null && getHashDocument(documentGNS).equals(hashDocGNT)) {
						try {
							storageTransfertService.deleteStorageDocument(idArchive);
							LOG.info("{} - Transfert - Suppression du document {} de la GNS.", "transfertDoc",
									idArchive.toString());
						} catch (final DeletionServiceEx ex) {
					...
					try {
						...
						for (final StorageDocumentNote note : listeNotes) {
							try {
								storageTransfertService.addDocumentNote(documentGNS.getUuid(), note.getContenu(),
										note.getAuteur(), note.getDateCreation(), note.getUuid());
							} catch (final DocumentNoteServiceEx e) {
						...
						try {
							...
							if (docAttache != null) {
								storageTransfertService.addDocumentAttachment(documentGNS.getUuid(),
										docAttache.getName(), docAttache.getExtension(), docAttache.getContenu());
							}
						} catch (final StorageDocAttachmentServiceEx e) {
	...
	private StorageMetadata getDateDebutConservation(final UUID uuidDoc) throws TransfertException {
		...
		try {
			final List<StorageMetadata> metadatasFind = storageDocumentService
					.retrieveStorageDocumentMetaDatasByUUID(new UUIDCriteria(uuidDoc, desiredStorageMetadatas));
			...
			dateDebutConservationMeta = new StorageMetadata(
					StorageTechnicalMetadatas.DATE_DEBUT_CONSERVATION.getShortCode(), dateDebutConservation);
	...
	public List<StorageMetadata> getListeStorageMetadatasWithGel(UUID idArchive)
		...
		Map<String, MetadataReference> listeAllMeta = metadataReferenceDAO.getAllMetadataReferencesPourVerifDroits();
