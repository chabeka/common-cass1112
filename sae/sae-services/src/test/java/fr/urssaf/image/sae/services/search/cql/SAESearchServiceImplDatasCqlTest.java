package fr.urssaf.image.sae.services.search.cql;
...
public class SAESearchServiceImplDatasCqlTest extends AbstractServiceCqlTest {
   ...
   private final int NB_DOCUMENTS_PAR_PAGE = 10;
   ...
   CaptureExistingUuuidException {
      ...
      Assert.assertEquals("Un seule métadonnée attendue : Note",
            1,
            listeDocs.get(0).getUMetadatas().size());
   ...
   private void validateNoteContent(final String contenu) {
      ...
      Assert.assertThat(contenu, CoreMatchers.containsString("\"auteur\":\"UTILISATEUR TEST\"}]"));
   }
