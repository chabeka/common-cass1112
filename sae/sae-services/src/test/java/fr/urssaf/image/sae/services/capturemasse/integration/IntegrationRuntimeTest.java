...
public class IntegrationRuntimeTest {
   ...
   private static final String ERREUR_ATTENDUE = "Une erreur interne à l'application est survenue "
         + "lors de la capture du document doc1.PDF. "
         + "Détails : erreur runtime créé par mock";
         ...
         if (CollectionUtils.isNotEmpty(docs.get(index).getErreurs()
            ...
            while (!erreurFound && indexErreur < listeErreurs.size()) {
               ...
               if (Constantes.ERR_BUL001.equals(erreurType.getCode())
                     && ERREUR_ATTENDUE.equalsIgnoreCase(ERREUR_ATTENDUE)) {
                  erreurFound = true;
               }
               indexErreur++;
            }
