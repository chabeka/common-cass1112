...
public abstract class AbstractTraceSupport<T extends Trace, I extends TraceIndex> {
	...
	private final SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH'h'mm ss's' SSS'ms'", Locale.FRENCH);

	/**
	 * Création d'une trace dans le registre de sécurité. les champs suivants
	 * sont renseignés :
	 * <ul>
	...
	public void create(final T trace, final long clock) {
		...
		final ColumnFamilyUpdater<UUID, String> updater = tmpl.createUpdater(trace.getIdentifiant());
		...
		if (StringUtils.isNotBlank(trace.getContratService())) {
			getDao().writeColumnContratService(updater, trace.getContratService(), clock);
		}
		...
		final ColumnFamilyUpdater<String, UUID> indexUpdater = getIndexDao().createUpdater(journee);
		getIndexDao().writeColumn(indexUpdater, index.getIdentifiant(), index, clock);
		getIndexDao().update(indexUpdater);

		// Trace applicative
		getLogger().debug("{} - Trace ajoutée dans le {} : Id={}. Timestamp={}", new Object[] { prefix,
				getRegistreName(), trace.getIdentifiant(), dateFormat.format(trace.getTimestamp()) });
	 ...
	 * @param reversed
	 *            booleen indiquant si l'ordre décroissant doit etre
	 *            appliqué<br>
	 *            <ul>
	...
	public final List<I> findByDates(final Date startDate, final Date endDate, final int maxCount,
		...
		getLogger().debug("{} - Début", prefix);
		getLogger().debug("{} - Date de début : {}", prefix, dateFormat.format(startDate));
		getLogger().debug("{} - Date de fin : {}", prefix, dateFormat.format(endDate));
		...
		final Iterator<I> iterator = getIterator(sliceQuery, startUuid, endUuid, reversed);
		...
		try {
			...
			getLogger().warn(
					...
					new Object[] { prefix, dateFormat.format(startDate), startUuid, dateFormat.format(endDate),
							endUuid });
			throw ex;
		}
	...
	private long deleteRecords(final Iterator<I> iterator, final long clock) {
		...
		while (iterator.hasNext()) {
			getDao().mutatorSuppressionLigne(mutator, iterator.next().getIdentifiant(), clock);
			result++;
		}
		mutator.execute();
	...
	private T getTraceFromResult(final ColumnFamilyResult<UUID, String> result) {
		...
		if (result != null && result.hasResults()) {
			...
			trace.setContratService(result.getString(TraceRegTechniqueDao.COL_CONTRAT_SERVICE));
	 ...
	 *            le résultat
	 */
	protected abstract void completeTraceFromResult(T trace, ColumnFamilyResult<UUID, String> result);
	 ...
	 * @return l'iterateur
	 */
	protected abstract Iterator<I> getIterator(SliceQuery<String, UUID, I> sliceQuery);
	 ...
	 * @return l'iterateur
	 */
	protected abstract Iterator<I> getIterator(SliceQuery<String, UUID, I> sliceQuery, UUID startUuid, UUID endUuid,
			boolean reversed);
	 ...
	 *            time de l'insertion
	 */
	protected abstract void completeCreateTrace(ColumnFamilyUpdater<UUID, String> updater, T trace, long clock);
