 ...
 */
@Component
public class TraceRegTechniqueSupport extends AbstractTraceSupport<TraceRegTechnique, TraceRegTechniqueIndex> {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(TraceRegTechniqueSupport.class);
	 ...
	 */
	@Autowired
	public TraceRegTechniqueSupport(TraceRegTechniqueDao dao, TraceRegTechniqueIndexDao indexDao,
	 ...
	 */
	@Override
	protected void completeCreateTrace(ColumnFamilyUpdater<UUID, String> updater, TraceRegTechnique trace, long clock) {
	 ...
	 */
	@Override
	protected Iterator<TraceRegTechniqueIndex> getIterator(SliceQuery<String, UUID, TraceRegTechniqueIndex> sliceQuery,
			UUID startUuid, UUID endUuid, boolean reversed) {
		return new TraceRegTechniqueIndexIterator(sliceQuery, startUuid, endUuid, reversed);
	}
