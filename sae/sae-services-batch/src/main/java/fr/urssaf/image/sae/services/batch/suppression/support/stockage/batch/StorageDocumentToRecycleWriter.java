...
ItemWriter<StorageDocument> {
  ...
  public final void moveToRecycleBeanStorageDocument(final UUID uuidJob,
    ...
    StorageMetadata metadataISI = null;
    StorageMetadata metadataDMC = null;
    StorageMetadata metadataIFI = null;
    ...
    for (final StorageMetadata metadata : storageDocument.getMetadatas()) {
      ...
      if (Constantes.CODE_COURT_META_ID_SUPPRESSION.equals(metadata.getShortCode())) {
        metadataISI = metadata;
      } else if (Constantes.CODE_COURT_META_DATE_CORBEILLE.equals(metadata.getShortCode())) {
        metadataDMC = metadata;
      } else if (StorageTechnicalMetadatas.ID_TRANSFERT_MASSE_INTERNE.getShortCode().equals(metadata.getShortCode())) {
        metadataIFI = metadata;
      }
    }
    // Si cas transfert de masse, on met pas la meta CODE_COURT_META_ID_SUPPRESSION
    // car on ne peut avoir un id de transfertMasseInterne et un idSuppressionMasseInterne
    // à la fois

    // si l'idTranfertMasseInterne est non null alors on est dans un transfert de masse
    if (metadataIFI != null) {
      modifiedMetadatas.add(metadataIFI);
      modifiedMetadatas.add(metadataDMC);
    } else {
      modifiedMetadatas.add(metadataISI);
      modifiedMetadatas.add(metadataDMC);
    }
