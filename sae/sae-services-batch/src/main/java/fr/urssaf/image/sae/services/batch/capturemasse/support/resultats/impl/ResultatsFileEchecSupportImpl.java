...
public class ResultatsFileEchecSupportImpl implements ResultatsFileEchecSupport {
  ...
  public final void writeResultatsFile(final File ecdeDirectory,
                                       ...
                                       final String batchModeTraitement, final ConcurrentLinkedQueue<?> listIntDocs) {

    writeResultatsFile(sommaireFile,
                       erreur,
                       nombreDocsTotal,
                       nbDocumentsIntegres,
                       batchModeTraitement,
   ...
   * @param isVirtual
   */
  private void writeResultatsFile(final File sommaireFile,
                                  final CaptureMasseErreur erreur, final int nombreDocsTotal,
                                  final int nbDocumentsIntegres, final String batchModeTraitement,
                                  final ConcurrentLinkedQueue<?> listIntDocs, final boolean isVirtual) {

    final String trcPrefix = "writeResultatsFile()";
    ...
    if (sommaireFile != null && sommaireFile.getParentFile() != null) {
      final File resultats = new File(sommaireFile.getParentFile(), "resultats.xml");

      final XmlReader reader = new XmlReader(sommaireFile);
      int index = 0;
      ...
      while (index < configuration.getNbreEssaiMax() && !writeFinished) {
        ...
        catch (final EcdePermissionException exception) {
      ...
      storedEx = new Throwable("Le sommaire est obligatoire pour la création du fichier résultat");
    }
  ...
  private void ecrireFichierResultat(final XmlReader reader,
                                     ...
                                     final boolean isVirtual, final ConcurrentLinkedQueue<?> listIntDocs) {

    ecrireEntete(nombreDocsTot,
                 nombreDocsIntegres,
                 batchModeTraitement,
                 isVirtual);

    if (Constantes.BATCH_MODE.TOUT_OU_RIEN.getModeNom()
                                          .equals(
                                                  batchModeTraitement)) {
      reader.initStream();
      this.ecrireFichierResultatDocumentsNonIntegres(reader,
                                                     isVirtual,
                                                     erreur,
                                                     false);
    } else if (Constantes.BATCH_MODE.PARTIEL.getModeNom()
                                            .equals(
                                                    batchModeTraitement)) {
      ...
      if (listIntDocs != null && !listIntDocs.isEmpty()) {
        ...
        ecrireFichierResultatDocumentsIntegres(reader,
                                               isVirtual,
                                               erreur,
                                               listIntDocs);
      } else {
  ...
  private void ecrireFichierResultatDocumentsIntegres(final XmlReader reader,
                                                      final boolean isVirtual, final CaptureMasseErreur erreur,
    ...
    final ConcurrentLinkedQueue<TraitementMasseIntegratedDocument> listIntDocTemp = new ConcurrentLinkedQueue<>();
    listIntDocTemp
    ...
    while (reader.hasNext()) {
      ...
      if (xmlEvent.getEventType() != XMLStreamConstants.COMMENT) {
        if (xmlEvent.isStartElement()) {
          startElement = xmlEvent.asStartElement();
          name = startElement.getName().getLocalPart();

          if (isVirtual) {
            indexReference = startTagVirtuelPartiel(name,
                                                    reader,
                                                    erreur,
                                                    indexReference,
                                                    listIntDocTemp);
            ...
            if ("documents".equals(name)) {
              ...
              indexReference = startTagDocsIntegres(name,
                                                    reader,
                                                    listIntDocTemp,
                                                    indexReference);
            }
   ...
   */
  private void ecrireFichierResultatDocumentsNonIntegres(final XmlReader reader,
                                                         final boolean isVirtual, final CaptureMasseErreur erreur,
                                                         final ConcurrentLinkedQueue<?> listIntDocs, final boolean addmetadatas) {
    XMLEvent xmlEvent = null;
    StartElement startElement;
    EndElement endElement;
    String name;
    ...
    while (reader.hasNext()) {
      ...
      if (xmlEvent.isStartElement()) {
        ...
        if (isVirtual) {
          indexReference = startTagVirtuel(name,
                                           reader,
                                           erreur,
                                           indexReference);
        } else {
          if ("documents".equals(name)) {
            staxUtils.addStartTag(NON_INTEGRATED_DOCUMENTS,
                                  PX_RES,
                                  NS_RES);
          } else {
            indexReference = startTagDocsNonIntegres(name,
                                                     reader,
                                                     erreur,
                                                     indexReference,
                                                     listIntDocs,
   ...
   */
  private void ecrireFichierResultatDocumentsNonIntegres(final XmlReader reader,
                                                         final boolean isVirtual, final CaptureMasseErreur erreur, final boolean addmetadatas) {
    ecrireFichierResultatDocumentsNonIntegres(reader,
                                              isVirtual,
                                              erreur,
                                              null,
                                              addmetadatas);
  ...
  private void endElementVirtuel(final String name) {
    if ("documentsVirtuels".equals(name)) {
      ...
      staxUtils.addEndTag(NON_INTEGRATED_VIRT_DOC,
                          PX_SOMRES,
                          NS_SOMRES);
  ...
  private IndexReference startTagDocsNonIntegres(final String name,
    ...
    if (CHEMIN_FICHIER.equals(name)) {
      ...
      if (!isDocumentDansListe(listIntDocs, indexReference, xmlEvent)) {
        reference = addTagsNonIntegratedDocumentByTagName(
                                                          CHEMIN_FICHIER,
                                                          xmlEvent,
                                                          erreur,
                                                          indexReference,
   ...
   *         false sinon.
   */
  private boolean isDocumentDansListe(final ConcurrentLinkedQueue<?> listIntDocs, final IndexReference indexReference, final XMLEvent xmlEvent) {
    TraitementMasseIntegratedDocument document = null;
    if (listIntDocs != null && !listIntDocs.isEmpty()) {
      document = getDocumentInListByXmlEvent(xmlEvent,
                                             listIntDocs,
                                             indexReference);
    }
   ...
   */
  private IndexReference addTagsNonIntegratedDocumentByTagName(final String baliseName,
                                                               final XMLEvent xmlEvent, final CaptureMasseErreur erreur,
                                                               final IndexReference indexReference, final XmlReader reader, final boolean addmetadatas) {
    final int value = indexReference.getDocIndex() + 1;
  ...
  private IndexReference startTagDocsIntegres(final String name, final XmlReader reader,
    ...
    if (CHEMIN_FICHIER.equals(name)) {
      indexReference = addIntegratedDocumentByTagName(CHEMIN_FICHIER,
                                                      reader,
                                                      listIntDocs,
                                                      indexReference);
    } else if (UUID_FICHIER.equals(name)) {
      indexReference = addIntegratedDocumentByTagName(UUID_FICHIER,
                                                      reader,
                                                      listIntDocs,
                                                      indexReference);
    }
   ...
   *          Index de référence
   */
  private IndexReference addIntegratedDocumentByTagName(final String tagName, final XmlReader reader,
                                                        final ConcurrentLinkedQueue<?> listIntDocs, final IndexReference indexReference) {
    final XMLEvent xmlEvent = reader.peek();
    final TraitementMasseIntegratedDocument document = getDocumentInListByXmlEvent(xmlEvent, listIntDocs, indexReference);
    if (document != null) {
      addTagIntegratedDocumentByTagName(tagName,
                                        xmlEvent,
                                        document,
                                        reader);
    }
  ...
  private void addTagIntegratedDocumentByTagName(final String tagName,
                                                 ...
                                                 final TraitementMasseIntegratedDocument document, final XmlReader reader) {
  ...
  private IndexReference startTagVirtuel(final String name,
    ...
    if ("documentsVirtuels".equals(name)) {
      ...
      staxUtils.addStartElement(NON_INTEGRATED_VIRT_DOC,
                                PX_SOMRES,
                                NS_SOMRES);
  ...
  private IndexReference startTagVirtuelPartiel(final String name,
    ...
    if (CHEMIN_FICHIER.equals(name)) {
      final XMLEvent xmlEvent = reader.peek();
      final TraitementMasseIntegratedDocument document = getDocumentInListByXmlEvent(xmlEvent,
                                                                                     listIntDocs,
                                                                                     indexReference);
      final boolean isDocumentIntegrated = document != null;
   ...
   * @param reader
   */
  private void addMetadatas(final CaptureMasseErreur erreur, final int index,
    ...
    boolean end = xmlEvent.isEndElement()
        && METADONNEES.equals(xmlEvent.asEndElement()
                                      .getName()
                                      .getLocalPart());
  ...
  private void addMetadatas(final XmlReader reader) {
    try {
      copieXmlEventReaderParNom(
                                trouverXmlEventReaderParNom(reader, METADONNEES),
                                METADONNEES);
    }
    catch (final ResultatEchecLectureXMLException e) {
  ...
  private void copieXmlEventReaderParNom(final XmlReader reader, final String nomXmlEvent) {
    ...
    XMLEvent xmlEvent = reader.peek();
    boolean end = xmlEvent.isEndElement()
        && nomXmlEvent.equals(xmlEvent.asEndElement()
                                      .getName()
                                      .getLocalPart());
  ...
  private XmlReader trouverXmlEventReaderParNom(final XmlReader reader,
    ...
    boolean baliseFind = xmlEvent.isStartElement()
        && nomXmlEvent.equals(xmlEvent.asStartElement()
                                      .getName()
                                      .getLocalPart());
  ...
  private void gestionElement(final XMLEvent xmlEvent) {
    ...
    if (xmlEvent.isStartElement()) {
      ...
      final String name = xmlEvent.asEndElement().getName().getLocalPart();
      staxUtils.addEndTag(name, PX_SOMRES, NS_SOMRES);

    } else if (xmlEvent.isCharacters()) {
      final String value = xmlEvent.asCharacters().getData();
   ...
   */
  private void ecrireEntete(final int nombreDocs, int nombreDocsIntegres,
                            final String batchModeTraitement, final boolean isVirtual) {
    ...
    final int nombreNonIntegrated = nombreDocs - nombreDocsIntegres;
    ...
    staxUtils.createTag("initialDocumentsCount",
                        countStandard,
                        PX_RES,
                        NS_RES);
    staxUtils.createTag("integratedDocumentsCount",
                        countIntegrated,
                        PX_RES,
                        NS_RES);
    staxUtils.createTag("nonIntegratedDocumentsCount",
                        countNonIntegrated,
                        PX_RES,
                        NS_RES);
    staxUtils.createTag("initialVirtualDocumentsCount",
                        countVirtual,
                        PX_RES,
                        NS_RES);
    staxUtils.createTag("integratedVirtualDocumentsCount",
                        countIntegratedVirtual,
                        PX_RES,
                        NS_RES);
   ...
   * @param xmlEvent
   */
  private void addErreur(final CaptureMasseErreur erreur, final int index,
                         final XMLEvent xmlEvent) {
    ...
    if (erreur.getListIndex().contains(index)) {
      ...
      for (int i = 0; i < erreur.getListIndex().size(); i++) {
        ...
        if (index == currIndex.intValue()) {
          final String code = erreur.getListCodes().get(i);
          final String messageErreur = erreur.getListException().get(i);
          String message = null;
   ...
   * @param erreur
   */
  private void addErreurVirtuelle(final CaptureMasseErreur erreur, final int index) {
   ...
   * @param erreur
   */
  private void addErreurReference(final CaptureMasseErreur erreur, final int index) {
  ...
  public final void writeVirtualResultatsFile(final File ecdeDirectory,
                                              final File sommaireFile, final CaptureMasseErreur erreur, final int totalDocuments,
                                              final int nbDocumentsIntegres, final String batchModeTraitement,
                                              final ConcurrentLinkedQueue<?> listIntDocs) {
  ...
  private class IndexReference {
     ...
     */
    public IndexReference(final int docIndex, final int refIndex) {
