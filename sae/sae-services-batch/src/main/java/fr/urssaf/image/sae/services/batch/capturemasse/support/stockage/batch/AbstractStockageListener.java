...
AbstractListener {
  ...
  protected final void beforeChunk() {
    while (Boolean.TRUE.equals(getExecutor().isInterrupted())) {
      try {
  ...
  protected final ExitStatus specificAfterStepOperations() {
    ...
    if (CollectionUtils.isNotEmpty(errorMessageList) && !isModePartielBatch()) {
  ...
  private ExitStatus stockageListener(
    ...
    final String trcPrefix = "stockageListener()";
    ...
    try {
      ...
      if (message == null) {
        message = "Exception inattendue : " + e.getClass();
      }
