...
public abstract class AbstractListener {
	...
	private static final ExitStatus FAILED_FIN_BLOQUANT = new ExitStatus("FAILED_FIN_BLOQUANT");
	...
	public final void beforeStep(final StepExecution stepExecution) {
		...
		batchMode = (String) getStepExecution().getJobExecution().getExecutionContext()
				.get(Constantes.BATCH_MODE_NOM_REDIRECT);
		repriseActif = getStepExecution().getJobParameters().getString(Constantes.TRAITEMENT_REPRISE) != null && Boolean
				.valueOf((String) getStepExecution().getJobParameters().getString(Constantes.TRAITEMENT_REPRISE));
	...
	/**
	 * @return la liste des codes erreurs stockée dans le contexte d'execution
	 *         du job
	 */
	@SuppressWarnings("unchecked")
	protected final ConcurrentLinkedQueue<String> getCodesErreurListe() {
		ExecutionContext jobExecution = getStepExecution().getJobExecution().getExecutionContext();
		return (ConcurrentLinkedQueue<String>) jobExecution.get(Constantes.CODE_EXCEPTION);
	}
	...
	protected final ConcurrentLinkedQueue<Integer> getIndexErreurListe() {
		ExecutionContext jobExecution = getStepExecution().getJobExecution().getExecutionContext();
		return (ConcurrentLinkedQueue<Integer>) jobExecution.get(Constantes.INDEX_EXCEPTION);
	}
	...
	protected final ConcurrentLinkedQueue<String> getErrorMessageList() {
		ExecutionContext jobExecution = getStepExecution().getJobExecution().getExecutionContext();
		return (ConcurrentLinkedQueue<String>) jobExecution.get(Constantes.DOC_EXCEPTION);
	}
	...
	protected final ConcurrentLinkedQueue<Integer> getIndexReferenceErreurListe() {
		ExecutionContext jobExecution = getStepExecution().getJobExecution().getExecutionContext();
		return (ConcurrentLinkedQueue<Integer>) jobExecution.get(Constantes.INDEX_REF_EXCEPTION);
	}
	...
	protected final ConcurrentLinkedQueue<Integer> getIndexRepriseDoneListe() {
		ExecutionContext jobExecution = getStepExecution().getJobExecution().getExecutionContext();
		return (ConcurrentLinkedQueue<Integer>) jobExecution.get(Constantes.INDEX_DOCUMENT_DONE);
	}
	...
	protected boolean isModePartielBatch() {
		return batchMode != null && Constantes.BATCH_MODE.PARTIEL.getModeNomCourt().equals(batchMode);
	}
