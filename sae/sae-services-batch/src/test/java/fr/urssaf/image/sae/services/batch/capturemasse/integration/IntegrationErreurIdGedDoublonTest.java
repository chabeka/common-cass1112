...
public class IntegrationErreurIdGedDoublonTest {

  private static final String LOG_WARN = "Génération de secours du log ERROR de rollback par procédure d'exploitation car il n'a pas été généré par le job de capture_masse";
  ...
  private void initDatas() throws IOException {
    ...
    final ClassPathResource resSommaire = new ClassPathResource(
                                                                "testhautniveau/erreurIdGedDoublon/sommaire.xml");
    ...
    final ClassPathResource resAttestation1 = new ClassPathResource(
                                                                    "testhautniveau/erreurIdGedDoublon/documents/doc1.PDF");
  ...
  private void checkFiles() throws IOException {
    ...
    final ClassPathResource resultatAttendu = new ClassPathResource(
                                                                    "testhautniveau/erreurIdGedDoublon/resultats-attendu.xml");
  ...
  private void checkLogs() {
    ...
    Assert.assertEquals("le log doit être de niveau ERROR",
                        Level.ERROR,
                        event.getLevel());
  ...
  private void checkTracabilite(final UUID idTdm, final URI urlSommaire) {
    final String traceValue = "La métadonnée IdGed est en doublon dans le sommaire.xml. Une procédure d'exploitation doit être initialisée afin de rejouer le traitement en echec.";
    traceAssertUtils
                    .verifieTraceCaptureMasseDansRegTechnique(
                                                              idTdm,
                                                              urlSommaire,
                                                              Arrays
                                                                    .asList(traceValue, traceValue, traceValue, traceValue));
