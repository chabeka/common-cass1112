...
public class IdentificationServiceImpl implements IdentificationService {
  ...
  public IdentificationServiceImpl(
                                   ...
                                   @Value("${sae.referentiel.format.cache}") final int cacheDuration,
    ...
    identifiers = CacheBuilder.newBuilder().refreshAfterWrite(cacheDuration,
                                                              TimeUnit.MINUTES).build(new CacheLoader<String, Identifier>() {
                                                                ...
                                                                @Override
                                                                public Identifier load(final String identifiant) {
                                                                  LOGGER
                                                                  .debug(
  ...
  private void populateCache() {
    ...
    for (final FormatFichier format : allFormat) {
      if (StringUtils.isNotEmpty(format.getIdentificateur())) {
        final Identifier identificateur = IdentificationServiceImpl.this.applicationContext.getBean(
   ...
   */
  @Override
  public final IdentificationResult identifyFile(final String idFormat, final File fichier)
      throws IdentifierInitialisationException, UnknownFormatException,
      IOException {

    // Traces debug - entrée méthode
    final String prefixeTrc = "identifyFile()";
    LOGGER.debug(LOG_DEBUT, prefixeTrc);

    // On récupère le nom du bean qui doit réaliser l'identification
    final String nomBeanIdentifier = lectureNomBeanIdentifier(idFormat);

    // On récupère le bean correspondant
    final Identifier identifier = recupereBeanIdentifier(nomBeanIdentifier);
    ...
    final IdentificationResult identifResult = identifier.identifyFile(idFormat,
                                                                       fichier);
   ...
   */
  @Override
  public final IdentificationResult identifyStream(final String idFormat,
                                                   final InputStream stream, final String nomFichier) throws UnknownFormatException,
  IdentifierInitialisationException {

    // Traces debug - entrée méthode
    final String prefixeTrc = "identifyStream()";
    LOGGER.debug(LOG_DEBUT, prefixeTrc);
    ...
    final IdentificationResult identifResult = identifier.identifyStream(idFormat,
                                                                         stream, nomFichier);
  ...
  private String lectureNomBeanIdentifier(final String idFormat)
      throws UnknownFormatException, IdentifierInitialisationException {

    // Traces debug - entrée méthode
    final String prefixeTrc = "lectureNomBeanIdentifier()";
    LOGGER.debug(LOG_DEBUT, prefixeTrc);
    ...
    if (formatFichier == null) {
      ...
      final String nomBean = formatFichier.getIdentificateur();
  ...
  private Identifier recupereBeanIdentifier(final String nomBeanIdentifier)
      throws IdentifierInitialisationException {

    // Traces debug - entrée méthode
    final String prefixeTrc = "recupereBeanIdentifier()";
    LOGGER.debug(LOG_DEBUT, prefixeTrc);
