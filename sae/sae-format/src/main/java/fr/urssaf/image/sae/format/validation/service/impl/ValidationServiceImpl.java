...
public final class ValidationServiceImpl implements ValidationService {
  ...
  public ValidationServiceImpl(
    ...
    validators = CacheBuilder.newBuilder().refreshAfterWrite(cacheDuration,
                                                             TimeUnit.MINUTES).build(new CacheLoader<String, Validator>() {
                                                               ...
                                                               @Override
                                                               public Validator load(final String identifiant) {
                                                                 LOGGER
                                                                 .debug(
  ...
  private void populateCache() {
    ...
    for (final FormatFichier format : allFormat) {
      if (StringUtils.isNotEmpty(format.getValidator())) {
        final Validator validateur = ValidationServiceImpl.this.applicationContext.getBean(
   ...
   */
  @Override
  public ValidationResult validateFile(final String idFormat, final File file)
      throws UnknownFormatException, ValidatorInitialisationException,
      ValidatorUnhandledException {

    // Traces debug - entrée méthode
    final String prefixeTrc = "validateFile()";
    LOGGER.debug(LOG_DEBUT, prefixeTrc);

    // On récupère le nom du bean qui doit réaliser la validation
    final String nomBeanValidator = lectureNomBeanValidator(idFormat);

    // On récupère le bean correspondant
    final Validator validator = recupereBeanValidator(nomBeanValidator);
    ...
    } catch (final FormatValidationException except) {
   ...
   */
  @Override
  public ValidationResult validateStream(final String idFormat, final InputStream stream)
      throws UnknownFormatException, ValidatorInitialisationException,
      IOException, ValidatorUnhandledException {

    // Traces debug - entrée méthode
    final String prefixeTrc = "validateStream()";
    LOGGER.debug(LOG_DEBUT, prefixeTrc);
  ...
  private String lectureNomBeanValidator(final String idFormat)
      throws UnknownFormatException, ValidatorInitialisationException {

    // Traces debug - entrée méthode
    final String prefixeTrc = "lectureNomBeanValidator()";
    LOGGER.debug(LOG_DEBUT, prefixeTrc);
    ...
    if (formatFichier == null) {
      ...
      final String nomBean = formatFichier.getValidator();
  ...
  private Validator recupereBeanValidator(final String nomBeanValidator)
      throws ValidatorInitialisationException {

    // Traces debug - entrée méthode
    final String prefixeTrc = "recupereBeanValidator()";
    LOGGER.debug(LOG_DEBUT, prefixeTrc);
