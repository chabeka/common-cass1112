...
public final class PdfaIdentifierImpl implements Identifier {
   ...
   */
  @Override
  public IdentificationResult identifyFile(final String idFormat, final File fichier)
      throws IOException {
    ...
    final IdentificationResult result = identifyFile(fichier);
  ...
  private IdentificationResult identifyFile(final File fichier) throws IOException {

    final String prefixeTrc = "identifyFile()";

    // Initialisation des résultats de l'analyse
    final IdentificationResult identificationResult = new IdentificationResult();
    final List<EtapeEtResultat> listeEtapeResult = new ArrayList<>();
    ...
    if (StringUtils.isBlank(puuid)) {
      ...
      if (StringUtils.equalsIgnoreCase(Constantes.FMT_354, puuid)) {
        ...
        final List<String> compatibles = saeFormatCompatible
        ...
        if (CollectionUtils.isEmpty(compatibles)) {
          ...
          // formats
          // comptabiles
          final boolean compatible = compatibles.contains(puuid);
          if (compatible) {
   ...
   */
  @Override
  public IdentificationResult identifyStream(final String idFormat,
    ...
    try {
      ...
      final File createdFile = File.createTempFile("sae_identifyStream_pdfa_",
                                                   extension);
  ...
  private void checkIdFormat(final String idFormat) {
