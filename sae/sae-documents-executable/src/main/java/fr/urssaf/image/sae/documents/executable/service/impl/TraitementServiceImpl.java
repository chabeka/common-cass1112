...
public class TraitementServiceImpl implements TraitementService {
  ...
  private void waitFinTraitementValidation(
                                           final FormatValidationParametres parametres, final int nbDocTraites,
                                           final FormatValidationPoolThreadExecutor executor, final int nbDocErreurIdent) {
    if (parametres.getModeVerification() != MODE_VERIFICATION.IDENTIFICATION) {
      executor.shutdown();
  ...
  private List<String> getMetadonnees(
    ...
    if (metadonnees.isEmpty()) {
      Collections.addAll(metadonnees, Constantes.getMETADONNEES_DEFAULT());
    }
    return metadonnees;
  }
  ...
  private void lancerValiderFichier(
                                    ...
                                    final Document document, final File file) {
    if (parametres.getModeVerification() != MODE_VERIFICATION.IDENTIFICATION) {
  ...
  private boolean isTraitementTermine(
                                      ...
                                      final Iterator<Document> iteratorDoc, final int nbDocTraites) {
   ...
   */
  @Override
  public void addMetadatasToDocuments(final AddMetadatasParametres parametres) {

    // -- Overture connexion dfce
    getDfceService().ouvrirConnexion();

    final String requeteLucene = parametres.getRequeteLucene();
    final Map<String, String> metas = parametres.getMetadonnees();
  ...
  private void waitFinTraitementAddMetadatas(
                                             final AddMetadatasPoolThreadExecutor poolThead) {
    poolThead.shutdown();
  ...
  private void waitFinTraitementPurgeCorbeille(
                                               final PurgeCorbeillePoolThreadExecutor poolThread) {
    poolThread.shutdown();
   ...
   */
  @Override
  public void addMetadatasToDocumentsFromCSV(final AddMetadatasParametres parametres) {
    ...
    try {
      ...
      while ((nextLine = reader.readNext()) != null) {
        ...
        final UUID idDoc = UUID.fromString(nextLine[0]);
        final Document doc = getDfceService().getDocumentById(idDoc);
        final String codeMeta = nextLine[1];
        ...
        if (doc != null && doc.getCriterions(codeMeta).isEmpty()) {
          ...
          final AddMetadatasRunnable addMetasRun = new AddMetadatasRunnable(
   ...
   */
  @Override
  public void purgerCorbeille(final PurgeCorbeilleParametres parametres) {

    final String trcPrefix = "purgerCorbeille()";
    ...
    final Boolean isRunning = statusService.isPurgeRunning();
    ...
    try {
      final Date minDate = paramService.getPurgeCorbeilleDateDebutPurge();
      final Integer retentionDuration = paramService.getPurgeCorbeilleDuree();
