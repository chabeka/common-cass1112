...
public class TuGenererVi {
   ...
   public static final URI SERVICE_VISE = ConverterUtils
   ...
   @Test
   @Ignore
   public void vi_success() throws KeyStoreException, NoSuchAlgorithmException,
      ...
      Element assertion = service.genererAssertion(params, keystore,
            keystorePublicKeyAlias, KEYSTORE_PASSWORD);
   ...
   private static KeyStore getKeyStorePortailImage() throws KeyStoreException,
   ...
   private static void debugAssertion(Logger logger, Element assertion) {
   ...
   public void vi_success_V2() {
      ...
      try {
         ...
         String assertionNonSignee = getAssertionNonSignee(assertionId, issuer,
               recipient, audience, authnInstant, notOnOrAfter, notBefore, pagm);
   ...
   private String getAssertionNonSignee(String assertionId, String issuer,
         String recipient, String audience, String authnInstant,
         String notOnOrAfter, String notBefore, List<String> pagms)
         throws IOException {

      // Lecture du modèle d'assertion depuis les ressources de
      // src/test/resources
      ...
      lines = IOUtils.readLines(this.getClass().getClassLoader()
            .getResourceAsStream("modele_vi/modele_assertion_saml2.xml"));
      // }
      // finally {
      // if (fis!=null) {
      // fis.close();
      ...
      assertion = StringUtils
            .replace(assertion, "[AuthnInstant]", authnInstant);
      assertion = StringUtils
            .replace(assertion, "[NotOnOrAfter]", notOnOrAfter);
   ...
   private String construitListePagm(List<String> pagm) {
      ...
      for (String unPagm : pagm) {
         pagmList.add("<saml2:AttributeValue>" + unPagm
   ...
   private String getAssertionSignee(String assertionNonSignee) {
      ...
      try {
         ...
         String assertionSignee = XmlSignature.signeXml(IOUtils
               .toInputStream(assertionNonSignee), keystore, alias, password);
