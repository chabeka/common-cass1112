...
public class WebServiceVIServiceValidate {
  ...
  public final void creerVIpourServiceWeb(final JoinPoint joinPoint) {

    // récupération des paramétres
    @SuppressWarnings("unchecked")
    final List<String> pagm = (List<String>) joinPoint.getArgs()[0];
    final String issuer = (String) joinPoint.getArgs()[1];
    final KeyStore keystore = (KeyStore) joinPoint.getArgs()[INDEX_3];
    final String alias = (String) joinPoint.getArgs()[INDEX_4];
    final String password = (String) joinPoint.getArgs()[INDEX_5];
  ...
  public final void verifierVIdeServiceWeb(final JoinPoint joinPoint) {

    // récupération des paramétres
    final Element identification = (Element) joinPoint.getArgs()[0];
    final URI serviceVise = (URI) joinPoint.getArgs()[1];
    final VISignVerifParams signVerifParams = (VISignVerifParams) joinPoint
                                                                           .getArgs()[2];
