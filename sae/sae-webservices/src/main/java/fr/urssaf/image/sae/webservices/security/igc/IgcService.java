...
public class IgcService {
   ...
   */
  @Autowired
  public IgcService(final IgcConfigs igcConfigs, final TracesWsSupport tracesWsSupport,
                    @Value("${sae.certifs.loadOnStartup}") final boolean loadOnStartup) {

    this.igcConfigs = igcConfigs;
    certsAcRacine = new HashMap<>();
   ...
   * 
   */
  public final void chargementCertificatsACRacine() {

    LOG.info(AC_RACINE_LOAD);

    final List<String> certificats = new ArrayList<>();

    final List<String> fichiersAcRacine = new ArrayList<>();
    ...
    for (final IgcConfig igcConfig : igcConfigs.getIgcConfigs()) {
      ...
      try {
        ...
        final File crt = new File(igcConfig.getAcRacine());
  ...
  private void createCertifsAndCrl() throws LoadCertifsAndCrlException {
    ...
    if (mustLoad) {
      ...
      if (isFirstLoad) {
        ...
        final String dateFormatee = certifsAndCrl.getDateMajCrl().toString(fmt);

        LOG.info(TextUtils.getMessage(CRL_RELOAD, dateFormatee));
      }

      final List<X509CRL> crls = new ArrayList<>();
      final List<String> fichiersCrl = new ArrayList<>();
      final List<String> fichiersErreurCrl = new ArrayList<>();
  ...
  private void setCertifsAndCrl(final CertifsAndCrl certifsAndCrl) {

    this.certifsAndCrl = certifsAndCrl;
  }

  private static List<X509CRL> loadCRLResources(final String repertoireCRLs,
                                                final List<String> fichiersCrl, final List<String> fichiersErreurCrl)
                                                    throws LoadCertifsAndCrlException {

    final FileSystemResource repCRLs = new FileSystemResource(repertoireCRLs);
    final List<Resource> resources = ResourceUtils.loadResources(repCRLs,
                                                                 CRL_EXTENSIONS);

    final List<X509CRL> crls = new ArrayList<>();

    for (final Resource crl : resources) {

      try {
        LOG.debug("loading CRL:" + crl.getFilename());

        final InputStream input = crl.getInputStream();
        ...
        try {
          ...
          try {
            final File fileToDelete = crl.getFile();
