...
/**
 * Implémentation du service {@link JobLectureThriftService}
 */
@Service
public class JobLectureServiceThriftImpl implements JobLectureThriftService {
    ...
    */
   @Autowired
   public JobLectureServiceThriftImpl(final JobRequestSupport jobRequestSupport,
   ...
   public final JobRequest getJobRequest(final UUID jobRequestUUID) {
      ...
      final ColumnFamilyResult<UUID, String> result = jobRequestSupport.getJobRequestTmpl().queryColumns(jobRequestUUID);
   ...
   public final JobRequest getJobRequestNotNull(final UUID uuidJob) throws JobInexistantException {
      final JobRequest jobRequest = getJobRequest(uuidJob);
      if (jobRequest == null) {
   ...
   public final List<JobQueue> getNonTerminatedSimpleJobs(final String hostname) {
      ...
      final List<JobQueue> list = new ArrayList<>(colNames.size());
   ...
   public final List<JobRequest> getNonTerminatedJobs(final String key) {
      ...
      final List<JobQueue> jobQueues = getNonTerminatedSimpleJobs(key);

      final List<JobRequest> jobRequests = new ArrayList<>();

      for (final JobQueue jobQueue : jobQueues) {
         final JobRequest jobRequest = getJobRequest(jobQueue.getIdJob());
   ...
   public final List<JobHistory> getJobHistory(final UUID idJob) {
      ...
      final Collection<UUID> colNames = result.getColumnNames();
      final List<JobHistory> histories = new ArrayList<>(colNames.size());
   ...
   public final List<JobRequest> getAllJobs(final Keyspace keyspace, final int maxKeysToRead) {
      ...
      final HectorIterator<UUID, String> resultIterator = new HectorIterator<>(
            result);
      final List<JobRequest> list = new ArrayList<>();
    ...
    */
   @Override
   public final List<UUID> getSemaphoredJobUuids() {

      final List<UUID> jobQueues = new ArrayList<>();

      final RangeSlicesQuery<String, UUID, String> rangeSliceQuery = jobsQueueSupport.createRangeSlicesQuery();
      rangeSliceQuery.setReturnKeysOnly();
      final QueryResult<OrderedRows<String, UUID, String>> result = rangeSliceQuery.execute();

      final SliceQuery<String, UUID, String> queryJobQueueCF = jobsQueueSupport.createSliceQuery();

      for (final Row<String, UUID, String> row : result.get().getList()) {
         if (row.getKey().contains(Constantes.PREFIXE_SEMAPHORE_JOB)) {
            queryJobQueueCF.setKey(row.getKey());
            final JobQueueIterator iterator = new JobQueueIterator(queryJobQueueCF);
            while (iterator.hasNext()) {
               jobQueues.add(iterator.next().getIdJob());
            }
         }
      }
   ...
   public UUID getJobRequestIdByJobKey(final byte[] jobKey) {
      ...
      final IndexedSlicesPredicate<UUID, String, byte[]> predicate = new IndexedSlicesPredicate<>(
            UUIDSerializer.get(),
            StringSerializer.get(),
      ...
      predicate.count(1);
      final ColumnFamilyResult<UUID, String> result = jobRequestSupport.getJobRequestTmpl().queryColumns(predicate);
