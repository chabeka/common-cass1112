...
public class JobsQueueSupportCql {
  ...
  // @Deprecated
   public final List<String> getHosts() {
    ...
    final Iterator<JobQueueCql> it = jobsQueueDaoCql.findAllWithMapper();
    while (it.hasNext()) {
      final JobQueueCql jobQueueCql = it.next();

      if (!jobQueueCql.getKey().equals("jobsWaiting")) {
        hosts.add(jobQueueCql.getKey());
      }
    }
      /*
   ...
   /**
    * Recupère la liste des UUIDS de job faisant l'objet d'un sémaphore
    * 
    * @return
    */
   public List<UUID> getSemaphoredJobUuids() {
      final List<UUID> listUuids = new ArrayList<>();
      final Iterator<JobQueueCql> it = jobsQueueDaoCql.getSemaphoredJobs();
      while (it.hasNext()) {
         listUuids.add(it.next().getIdJob());
      }
