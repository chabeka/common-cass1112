...
public class JobQueueServiceProcessExistingJobsTest {
  ...
  public void processExistingJobWithOldParam() throws JobDejaReserveException, JobInexistantException, LockTimeoutException {
    ...
    JobRequest jobRequest = jobLectureServiceCql.getJobRequest(idJobExistant);
    ...
    Assert.assertEquals("l'état est inattendu", JobState.STARTING, jobRequest.getState());
    ...
    final JobRequest jobRequestJobParam = jobLectureServiceCql.getJobRequest(idJobWithJobParam);
    Assert.assertEquals("l'état est inattendu", JobState.STARTING, jobRequestJobParam.getState());
