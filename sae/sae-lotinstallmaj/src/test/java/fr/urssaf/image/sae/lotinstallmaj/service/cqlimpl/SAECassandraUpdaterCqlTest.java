...
public class SAECassandraUpdaterCqlTest {
	...
	public void addTableTracesToSAE() {
		...
		try {
			...
			List<String> tableNamesBefore = UtilsColunmFalmilly.getTablesNames(saecf.getSession(), "\"SAE\"");
			
			saeUpd.createTablesTraces();
			
			List<String> tableNamesAfter = UtilsColunmFalmilly.getTablesNames(saecf.getSession(), "\"SAE\"");
			Assert.assertEquals("Les 9 tables de traces aurait du être crées en plus", tableNamesAfter, tableNamesBefore.size() +9); 
	...
	@Test
	public void deleteAllTablesTraces() {
		...
		try {
			...
			List<String> tableNamesAfter = UtilsColunmFalmilly.getTablesNames(saecf.getSession(), "\"SAE\"");
			Assert.assertEquals("Les 9 tables de traces doivent être supprimées de la base", tableNamesAfter, tableNamesBefore.size() - 9); 
	...
	public void connectTOSAE() {
		...
		try {
			...
			List<String> tableNames = UtilsColunmFalmilly.getTablesNames(saecf.getSession(), "\"SAE\"");
	...
	public void createAndDeleteJobSpringTables() {
		...
		List<String> tablesAfterCreate = UtilsColunmFalmilly.getTablesNames(saecf.getSession(), "\"SAE\"");
		Assert.assertEquals("La base doit contenir les 9 tables job Spring", tableNamesBefore.size(), tablesAfterCreate.size() + 9);
		
		saeUpd.deleteTablesJobSpring();
		
		List<String> tableAfterDelete = UtilsColunmFalmilly.getTablesNames(saecf.getSession(), "\"SAE\"");
		Assert.assertEquals("La base doit contenir les 9 tables job Spring", tableNamesBefore.size(), tableAfterDelete.size() - 9);
	}
}
