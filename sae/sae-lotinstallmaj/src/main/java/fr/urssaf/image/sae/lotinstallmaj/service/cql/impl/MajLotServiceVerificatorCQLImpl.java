...
public class MajLotServiceVerificatorCQLImpl implements MajLotServiceVerificator{
  ...
  private boolean verifyVersion32() {
    ...
    isOK = isOK && verifyIndexesCompositesFromPlatform(indexCodeCourt);
  ...
  private boolean verifyVersion33() {
    ...
    boolean isOK = false;
    LOG.info("Vérification de la version de la base");
    if (saeDao.getDatabaseVersion() < LotVersion.CASSANDRA_DFCE_200500.getNumVersionLot()) {
      return false;
    }
    LOG.info("Vérification des meta NomContact,PrenomContact");
  ...
  private boolean verifyVersion34() {
    ...
    isOK = refMetaInitService.findListMeta(null, codesLong);
    LOG.info("Vérification de la métadonnée DateNaissanceCotisant:{}", isOK ? "OK" : "KO");

    // vérifier la création des formats
    final List<String> formats = Arrays.asList("rtf", "eml");
    final boolean formatFichierOk = verifyFormatFichier(formats);
    LOG.info("Vérification des formats: rtf, eml : {}", formatFichierOk ? "OK" : "KO");
    isOK = isOK && formatFichierOk;

    // vérifier la creation de l'indexComposite
    final String indexCodeCourt = "cot&cop&dnc&SM_ARCHIVAGE_DATE&";
    final boolean indexCompositesOk = verifyIndexesCompositesFromPlatform(indexCodeCourt);
    isOK = isOK && indexCompositesOk;

    return isOK;
  }

  private boolean verifyIndexesCompositesFromPlatform(final String indexCodeCourt) {

    boolean indexCompositesOk=false;

    if (refMetaInitService.indexMustExistsPlatform(indexCodeCourt)) {
      indexCompositesOk = verifyIndexesComposites(indexCodeCourt);
      LOG.info("Vérification index composite {} : {}", indexCodeCourt, indexCompositesOk ? "OK" : "KO");
    }else {
      indexCompositesOk=true;
    }
    return indexCompositesOk;
  }
