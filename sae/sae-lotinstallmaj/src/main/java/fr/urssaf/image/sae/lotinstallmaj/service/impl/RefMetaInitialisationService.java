...
public final class RefMetaInitialisationService {
  ...
  protected static final String XSD_METADONNEES = "/xsd/metadata/Metadonnees.xsd";

  protected static final String XSD_INDEXES_COMPOSITES = "/xsd/metadata/IndexesComposites.xsd";

  // A MODIFIER LORS DES EVOLUTIONS DE FICHIERS !!
  private static final String FICHIER_METADONNEES = "Metadonnees.4.5.xml";

  protected static final String NOM_FICHIER_INDEX_COMPOSITE = "IndexesComposites4.5.xml";
  ...
  protected Map<String[], String> chargerFichierIdxCompositesGNT(final boolean aCreer) throws IOException, JAXBException, SAXException  {
    ...
    final List<IndexReference> indexes = getIndexes();
  ...
  /**
   * Récupération des index à partir des fichiers XML
   * 
   * @return
   * @throws IOException
   ...
   * @throws SAXException
   */
  public List<IndexReference> getIndexes() throws IOException, JAXBException, SAXException {
  ...
  public boolean indexMustExistsPlatform(final String indexCodeCourt) {
    boolean indexOk = false;
    String indexCodeCourtForXML = indexCodeCourt.replaceAll("&", ":");
    indexCodeCourtForXML = indexCodeCourtForXML.substring(0, indexCodeCourtForXML.length() - 1);
    List<IndexReference> indexes;
    try {
      indexes = getIndexes();
      for (final IndexReference index : indexes) {
        if (index.getComposition().equals(indexCodeCourtForXML)) {
          indexOk = typePlatform.equals("GNS") && index.getACreerGNS().equals("oui")
              || typePlatform.equals("GNT") && index.getACreerGNT().equals("oui");
          break;
        }
      }
    }
