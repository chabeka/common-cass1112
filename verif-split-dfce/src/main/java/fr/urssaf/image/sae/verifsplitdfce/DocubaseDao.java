...
public class DocubaseDao {
   ...
   public DocubaseDao(String hosts, String username, String password,
         String baseName) {
      super();
      this.hosts = hosts;
   ...
   private Keyspace getKeyspaceDocubaseFromKeyspace() {
      ...
      Cluster cluster = HFactory.getOrCreateCluster(
            "ClusterName-" + new Date().getTime(), hostConfigurator);
   ...
   private UUID getBaseUUIDByName(String nomBase, Keyspace keyspaceDocubase) {
      ...
      SliceQuery<String, String, byte[]> queryDocubase = HFactory
            .createSliceQuery(keyspaceDocubase, StringSerializer.get(),
                  StringSerializer.get(), BytesArraySerializer.get());
      queryDocubase.setColumnFamily("BasesReference");
      queryDocubase.setKey(nomBase);
      AllColumnsIterator<String, byte[]> iterColonne = new AllColumnsIterator<String, byte[]>(
            queryDocubase);
   ...
   public boolean verifExistanceIndex(String indexName) {
      boolean exist = false;
      if (indexName.equals("SM_UUID") || indexName.equals("SM_CREATION_DATE")
         ...
         SliceQuery<byte[], String, byte[]> queryDocubase = HFactory
               .createSliceQuery(keyspaceDocubase, BytesArraySerializer.get(),
                     StringSerializer.get(), BytesArraySerializer.get());
         queryDocubase.setColumnFamily("BaseCategoriesReference");
         queryDocubase
               .setKey(StringSerializer.get().toBytes(buffer.toString()));
   ...
   public boolean verifExistanceIndexComposite(String indexName) {
      boolean exist = false;
      ...
      queryDocubase.setKey(StringSerializer.get().toBytes(indexName));
      queryDocubase.setRange(null, null, false, 1);
   ...
   public String getTypeIndex(String indexName) {
      ...
      if (indexName.equals("SM_UUID")) {
         ...
         Keyspace keyspaceDocubase = getKeyspaceDocubaseFromKeyspace();
         SliceQuery<String, String, String> queryDocubase = HFactory
               .createSliceQuery(keyspaceDocubase, StringSerializer.get(),
                     StringSerializer.get(), StringSerializer.get());
         queryDocubase.setColumnFamily("CategoriesReference").setKey(indexName)
               .setColumnNames("categoryTypeENUM_ATTRIBUTE_VALUE_SUFFIXE");
         QueryResult<ColumnSlice<String, String>> resultDocubase = queryDocubase
               .execute();
         if (resultDocubase.get() != null
               && !resultDocubase.get().getColumns().isEmpty()) {
            type = resultDocubase.get()
                  .getColumnByName("categoryTypeENUM_ATTRIBUTE_VALUE_SUFFIXE")
                  .getValue();
         }
      }
      return type;
   ...
   public void checkIndexInIndexReference(String indexName)
      ...
      while (iterColonne.hasNext()) {
         ...
         if (colonne.getName().matches("rangeIndexes.[0-9]*.key")) {
            ...
            if (key.longValue() > maxKey.longValue()) {
               maxKey = key;
               numRowMaxKey = ConvertUtils
                     .convertByteToLong(colonne.getValue());
            }
            ...
            JSONObject object = new JSONObject(StringSerializer.get()
                  .fromBytes(colonne.getValue()));
      ...
      if (compteur == 0) {
         LOGGER.error("Cet index ne comporte aucun range");
         error = true;
      } else {
         ...
         if (maxKey.longValue() > 0) {
            ...
            // LOGGER.warn("Ne tient pas compte du range {} de la toute dernière key",
            // numRowMaxKey);
            listNumRow.remove(numRowMaxKey);
            if (!listNumRow.contains(numRowMaxKey)) {
               // LOGGER.warn("Le range {} n'est plus reference, donc on supprime les bornes",
               // numRowMaxKey);
         ...
         for (Long numRow : doublons.keySet()) {
            ...
            if (doublons.get(numRow) == 2) {
               LOGGER.warn(
                     "Doublon du range {} pour l'index {}. Ce n'est pas un problème",
                     numRow, indexName);
               index = 1;
            } else if (doublons.get(numRow) > 2) {
               LOGGER.error(
                     "Trop de ranges({}) pour la row {} pour l'index {}",
                     new Object[] { doublons.get(numRow), numRow, indexName });
         ...
         if (nbRange < listNumRow.size()) {
            LOGGER.error(
                  "Le nombre de range n'est pas correct pour l'index {}: {} au lieu de {}",
                  new Object[] { indexName, nbRange, listNumRow.size() });
            error = true;
         }
         ...
         for (String borneMin : bornes.keySet()) {
            if (!bornePrecedente.equals("")) {
               if (!bornes.get(borneMin)[0].equalsIgnoreCase(bornePrecedente)) {
                  LOGGER.error(
                        "Valeur differente entre la borne precedente et le nouveau range : {} - {}",
                        bornePrecedente, bornes.get(borneMin)[0]);
                  error = true;
               }
            }
   ...
   public void verifRangesInTermInfoRange(String indexName) {
      ...
      List<Long> numRows = getIndexReferenceByIndexName(indexName, idBase,
            keyspaceDocubase);

      LOGGER.info("Lancement du comptage de l'index {} pour la base {} ({})",
            new String[] { indexName, baseName, idBase.toString() });
      ...
      for (Long numRow : numRows) {
         ...
         CountQuery<byte[], Composite> queryDocubaseTerm = HFactory
               .createCountQuery(keyspaceDocubase, BytesArraySerializer.get(),
                     CompositeSerializer.get());
         queryDocubaseTerm.setColumnFamily(cfName);
         queryDocubaseTerm.setKey(CompositeSerializer.get().toBytes(
               compositeKey));
         ...
         LOGGER.info("{} docs indexés sur la row {} de l'index {}",
               new String[] { Long.toString(nbCompteur), numRow.toString(),
                     indexName });
         total += nbCompteur;
      }

      LOGGER.info("{} docs indexés au total sur l'index {}", new String[] {
            Long.toString(total), indexName });
   }

   private List<Long> getIndexReferenceByIndexName(String indexName,
         UUID baseId, Keyspace keyspaceDocubase) {
