...
public final class MajLotServiceImpl implements MajLotService {
	...
	public void demarre(String nomOperation, String[] argSpecifiques) {
		...
		if (CODE_ACTIVITE.equalsIgnoreCase(nomOperation)) {
			...
			// 160600 :Mise en commentaire car si on créé une base du départ,
			// les
			// index
			// composite seront créés à partir de la nouvelle méthode
			// différenciant
			// GNT et GNS
		...
		} else if (CASSANDRA_DFCE_180900.equalsIgnoreCase(nomOperation)) {
			// Update keyspace SAE
			updateCassandra180900();
		} else if (CASSANDRA_DFCE_180901.equalsIgnoreCase(nomOperation)) {
			// Update keyspace SAE
			updateCassandra180901();
			// MAJ des métadonnées
			updateMetaDfce("META_180901");
		} else {
	...
	public void demarreUpdateDFCE(final String applicationConcernee) {
		...
		String applConcernee = StringUtils.isNotEmpty(applicationConcernee) ? applicationConcernee : null;
	...
	private void connectDfce() {
		serviceProvider.connect(dfceConfig.getLogin(), dfceConfig.getPassword(), dfceConfig.getUrlToolkit(),
				dfceConfig.getTimeout());
	}
	...
	private void updateCassandra180900() {
	...
	private void updateCassandra180901() {
		LOG.info("Début de l'opération : mise à jour du keyspace SAE pour le lot 180900");
		// Récupération de la chaîne de connexion au cluster cassandra
	...
	private void addIndexesCompositeToDfce(String operation, APPL_CONCERNEE gedConcernee) {
	 ...
	 * lorsqu'il utilise un des index composites, la requete lancee n'utilise
	 * pas le bon index composite (JIRA-154). Pour pallier temporaire ce
	 * probleme, nous allons supprimer cette index composite de docubase.
	 */

	// private void disableCompositeIndex() {
	//
	// RefMetaInitialisationService service = updater.getRefMetaInitService();
	// LOG.info("Début de l'opération : DISABLE_COMPOSITE_INDEX - Mise à jour de
	// DFCE");
	// DFCEUpdater dfceUpdater = new DFCEUpdater(cassandraConfig);
	// dfceUpdater.disableCompositeIndex(service
	// .getIndexesCompositesASupprimer());
	// LOG.info("Fin de l'opération : DISABLE_COMPOSITE_INDEX - Mise à jour de
	// DFCE");
	// }
	...
	private void disableCompositeIndex(APPL_CONCERNEE gedConcernee) {
		...
		if (APPL_CONCERNEE.GNS.equals(gedConcernee)) {
			dfceUpdater.disableCompositeIndex(service.getIndexesCompositesASupprimerGNS());
		} else if (APPL_CONCERNEE.GNT.equals(gedConcernee)) {
			dfceUpdater.disableCompositeIndex(service.getIndexesCompositesASupprimerGNT());
		}
