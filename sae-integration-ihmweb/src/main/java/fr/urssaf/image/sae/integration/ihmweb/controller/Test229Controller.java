...
import fr.urssaf.image.sae.integration.ihmweb.formulaire.Test229Formulaire;
...
/**
 *  229-CaptureMasse-OK-Virtuel-Un-Document-Complet 
 */
@Controller
@RequestMapping(value = "test229")
public class Test229Controller extends
AbstractTestWsController<Test229Formulaire> {

   /**
    * Nombre d'occurence attendu
    */
   private static final int COUNT_WAITED = 1;
   private static final int COMPTAGE_COUNT_WAITED = 2501;
   ...
   private String getDebutUrlEcde() {
      ...
      .construitUrlEcde(
            "SAE_INTEGRATION/20110822/CaptureMasse-229-CaptureMasse-OK-Virtuel-Un-DocumentComplet/");
   }
    ...
    */
   @Override
   protected final Test229Formulaire getFormulairePourGet() {

      Test229Formulaire formulaire = new Test229Formulaire();
      ...
      List<ConsultationFormulaire> consultFormulaireList = formulaire.getConsultFormulaireList();
      ...
      for(int i=0; i< getCasTest().getLuceneExempleList().size(); i++){
         ConsultationFormulaire formConsult = new ConsultationFormulaire(formulaire);
         formConsult.setCodeMetadonnees(codeMetaConsult);
         consultFormulaireList.add(formConsult);
      }
    ...
    */
   @Override
   protected final void doPost(Test229Formulaire formulaire) {
      ...
      for(ConsultationFormulaire f :formulaire.getConsultFormulaireList()){
         f.setParent(formulaire);
      }
      if ("1".equals(etape)) {
         ...
         etape4Consultation(formulaire, new Integer(sousEtape));
   ...
   private void etape3Recherche(Test229Formulaire formulaire, int sousEtape) {
      ...
      if (!TestStatusEnum.Echec.equals(resultatTest.getStatus())) {
         ...
         // Initialise le formulaire de consultation
         formulaire.getConsultFormulaireList().get(sousEtape).setIdArchivage(
   ...
   private void etape4Consultation(Test229Formulaire formulaire, int sousEtape) {
      ...
      valeursMetaAttendus.add(new MetadonneeValeur("Denomination","Test 229-CAPTUREMASSE-OK-VIRTUEL-UN-DOCUMENT-COMPLET"));
      if (sousEtape == 0) {
         valeursMetaAttendus.add(new MetadonneeValeur("NumeroRecours","1"));
      } else if (sousEtape == 1) {
         valeursMetaAttendus.add(new MetadonneeValeur("NumeroRecours","2501"));
      } 
      ...
      getConsultationTestService()
         .appelWsOpConsultationReponseCorrecteAttendue(
               formulaire.getUrlServiceWeb(),
               formulaire.getConsultFormulaireList().get(sousEtape),
               null,
               codeMetaAttendues,
               valeursMetaAttendus);
      ...
      // Si le test n'est pas en échec, alors on peut le passer en succès,
      // car tout a pu être vérifié
      ResultatTest resultatTest = formulaire.getConsultFormulaireList().get(sousEtape).getResultats();
      if (!TestStatusEnum.Echec.equals(resultatTest.getStatus())) {
         resultatTest.setStatus(TestStatusEnum.Succes);
      }
   ...
   private void etape5Comptages(Test229Formulaire formulaire) {
   ...
   private MetadonneeValeurList getValeursAttendues(int numeroResultat) {
      ...
      MetadonneeValeurList valeursAttendues = new MetadonneeValeurList();
      valeursAttendues.add("Denomination","Test 229-CAPTUREMASSE-OK-VIRTUEL-UN-DOCUMENT-COMPLET");
      
      if (numeroResultat==0) {
         valeursAttendues.add("NumeroRecours","1");
      } else if (numeroResultat==1) {
         valeursAttendues.add("NumeroRecours","2501");         
      } else {
