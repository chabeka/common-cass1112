...
public class IndexCategoriesJob extends AbstractTestCaseCreateAndPrepareBase {
	...
	Base updateBase = serviceProvider.getBaseAdministrationService()
		.updateBase(base);
	...
	for (UUID uuid : uuids) {
	    SearchQuery searchQuery = toolkitFactory.createMonobaseQuery(
		    category1.getName() + ":" + uuid, updateBase);
	...
	String parameters = "category.names=" + category1.getName()
