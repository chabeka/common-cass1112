...
public class CRTL77Test extends AbstractCRTLTest {
	...
	try {
	    ...
	    document = serviceProvider.getStoreService().updateDocument(
		    document);
    ...
    public void testQueryCREATION_DATE() throws ExceededSearchLimitException,
	    SearchQueryParseException {
	SearchService searchService = serviceProvider.getSearchService();
	String creationDateAsString = searchService.formatDate(CREATION_DATE,
		DateFormat.DATETIME);

	SearchQuery searchQuery = ToolkitFactory.getInstance()
		.createMonobaseQuery(
			SystemFieldName.SM_CREATION_DATE + ":"
				+ creationDateAsString, base);
    ...
    @Test
    public void testQueryMODIFICATION_DATE()
	    throws ExceededSearchLimitException, SearchQueryParseException {
	SearchService searchService = serviceProvider.getSearchService();
	String modificationDateAsString = searchService.formatDate(
		MODIFICATION_DATE, DateFormat.DATETIME);

	SearchQuery searchQuery = ToolkitFactory.getInstance()
		.createMonobaseQuery(
			SystemFieldName.SM_MODIFICATION_DATE + ":"
				+ modificationDateAsString, base);
