...
import fr.urssaf.image.parser_opencsv.application.exception.EmptyFileException;
    ...
    while ((nextLine = csvReader.readNext()) != null) {
      ...
      try {
        ...
        if (!missingMetadatas.isEmpty()) {
          ...
          final File existefile = new File(sourceFile);
          if (existefile.exists()) {

            // Verifier si le fichier est vide
            if (existefile.length() == 0) {
              throw new EmptyFileException("Le fichier binaire est vide");
            }
      ...
      catch (final CorrespondanceException | CorrespondanceFormatException | ParseException | HashInexistantException | CountNbrePageFileException |
          UnknownHashCodeEx | EmptyFileException e) {
