...
public class BuildServiceValidation {
   ...
   public final void buildStorageLuceneCriteriaValidation(
      ...
      Validate.notNull(luceneQuery, MappingMessageHandler
            .getMessage("build.lucene.required"));
      if (limit <= 0) {
         throw new IllegalArgumentException(MappingMessageHandler
               .getMessage("build.limit.required"));
      }
      Validate.notNull(metadatas, MappingMessageHandler
            .getMessage("build.metadata.required"));
   }
   ...
   public final void buildStorageUuidCriteriaValidation(final UUID uuid,
         final List<SAEMetadata> metadatas) {
      Validate.notNull(uuid, MappingMessageHandler
            .getMessage("build.uuid.required"));
      Validate.notNull(metadatas, MappingMessageHandler
            .getMessage("build.metadata.required"));
   }
   ...
   public final void buildUntypedDocumentValidation(final DataHandler content,
         final Map<String, String> metadatas) {
      Validate.notNull(content, MappingMessageHandler
            .getMessage("build.content.required"));
      Validate.notNull(metadatas, MappingMessageHandler
            .getMessage("build.metadata.required"));
   }
}
