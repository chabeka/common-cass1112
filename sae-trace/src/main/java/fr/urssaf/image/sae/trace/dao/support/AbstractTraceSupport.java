...
public abstract class AbstractTraceSupport<T extends Trace, I extends TraceIndex> {
  ...
  private final SimpleDateFormat dateFormat = new SimpleDateFormat(
                                                                   "yyyy-MM-dd HH'h'mm ss's' SSS'ms'",
                                                                   Locale.FRENCH);
   ...
   *          Nombre maximun de ligne d'evennement à supprimer
   * @return le nombre de traces purgées
   */
  public final long delete(final Date date, final long clock, final int nbMaxLigneEvtToDelete) {
    ...
    if (iterator.hasNext()) {
      ...
      nbTracesPurgees = deleteRecords(iterator, clock, nbMaxLigneEvtToDelete);
   ...
   */
  private long deleteRecords(final Iterator<I> iterator, final long clock, final int nbMaxLigneEvtToDelete) {
    long result = 0;
    int execMutatorSlice = 0;
    ...
    while (iterator.hasNext()) {
      getDao().mutatorSuppressionLigne(mutator,
                                       ...
                                       clock);
      if (execMutatorSlice >= nbMaxLigneEvtToDelete) {
        mutator.execute();
        execMutatorSlice = 0;
      } else {
        execMutatorSlice++;
  ...
  private T getTraceFromResult(final ColumnFamilyResult<UUID, String> result) {
    ...
    if (result != null && result.hasResults()) {
      ...
      final Date timestamp = result.getDate(AbstractTraceDao.COL_TIMESTAMP);

      trace = createNewInstance(idTrace, timestamp);

      trace.setCodeEvt(result.getString(AbstractTraceDao.COL_CODE_EVT));
