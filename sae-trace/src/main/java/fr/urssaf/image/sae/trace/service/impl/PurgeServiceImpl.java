...
public class PurgeServiceImpl implements PurgeService {
  ...
  @Value("${sae.purge.nombre.delete.bloc}")
  private int nbMaxLigneEvtToDelete;
  ...
  public final void purgerRegistre(final PurgeType typePurge) {
    ...
    final String trcPrefix = "purgerRegistre()";

    LOGGER.debug("{} - début", trcPrefix);
    loggerSupport.logPurgeDebut(LOGGER, trcPrefix, typePurge);

    final List<PurgeType> authorized = Arrays.asList(PurgeType.PURGE_EXPLOITATION,
                                                     PurgeType.PURGE_SECURITE,
    ...
    final Boolean isRunning = statusService.isPurgeRunning(typePurge);
    if (Boolean.TRUE.equals(isRunning)) {
      final String registre = getLibelleFromPurgeType(typePurge);
      throw new TraceRuntimeException(StringUtils.replace(
                                                          "La purge des registres {0} est déjà en cours",
                                                          "{0}",
                                                          registre));
    }
    ...
    try {
      ...
      final Date minDate = getDateFromPurgeType(typePurge);
  ...
  public final void purgerJournal(final PurgeType typePurge) {
    ...
    final String trcPrefix = "purgerJournal()";
    final List<PurgeType> authorized = Arrays.asList(PurgeType.PURGE_EVT);
    ...
    final Boolean isRunning = statusService.isPurgeRunning(typePurge);
    if (Boolean.TRUE.equals(isRunning)) {
      final String libelle = getLibelleFromPurgeType(typePurge);
      throw new TraceRuntimeException(StringUtils.replace(
                                                          "La purge du journal {0} est déjà en cours",
                                                          "{0}",
                                                          libelle));
    }
    ...
    try {
      ...
      final Date dateDerniereJourneePurgee = getDateFromPurgeType(typePurge);
      final Date premiereJourneeApurger = DateUtils.addDays(
                                                            dateDerniereJourneePurgee,
                                                            1);
      ...
      final Date lastDateJournalisation = getDateJournalisationFromPurgeType(typePurge);
  ...
  @SuppressWarnings("unchecked")
  private void purge(final RegService servicePurge, final Date minDate, final Date maxDate,
                     final PurgeType typePurge) {

    final String prefix = "purge()";
    LOGGER.debug("{} - debut de purge pour le type {}",
                 new Object[] {
                               prefix, typePurge.toString()});

    Date date = DateUtils.truncate(minDate, Calendar.DATE);
    final Date endDate = DateUtils.truncate(maxDate, Calendar.DATE);
    ...
    do {
      ...
      servicePurge.purge(date, nbMaxLigneEvtToDelete);
    ...
    LOGGER.debug("{} - fin de la purge pour le type {}",
                 new Object[] {
                               prefix, typePurge.toString()});

  }

  private void setDateFromPurgeType(final PurgeType typePurge, final Date date) {
  ...
  private Date getDateFromPurgeType(final PurgeType typePurge) {
  ...
  private Integer getDureeFomPurgeType(final PurgeType typePurge) {
  ...
  private String getLibelleFromPurgeType(final PurgeType purgeType) {
    String registre;
  ...
  private Date getDateJournalisationFromPurgeType(final PurgeType typePurge) {
