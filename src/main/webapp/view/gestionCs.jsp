...
<form:form id="cs" method="post" modelAttribute="csForm">
	...
	<c:if test="${not empty cs.pagms.pagm}">
		...
		<display:table name="${listePagms}" id="pagm" style="width:95%"
			...
			<display:column title="Compression pdf">
				<c:if test="${not empty pagm.compressionPdfActive}">
					<c:out value="${ihmexploit:booleanToOuiNon(pagm.compressionPdfActive)}" />
				</c:if>
			</display:column>
			
			<display:column title="Seuil compression pdf">
				<c:out value="${pagm.seuilCompressionPdf}" />
			</display:column>
	...
	<c:if test="${not empty droitModifCs}">
		<p>Modification de CS :</p>
		<display:table name="droitModifCs" id="modifCs" style="width: 95%"
			defaultorder="ascending" pagesize="${nbLignesTableau}"
			requestURI="/gestionCs.do">
			<display:column title="issuer" sortable="true">
				<c:out value="${modifCs.issuer}" />
			</display:column>
			<display:column title="Description" sortable="true">
				<c:out value="${modifCs.description}" />
			</display:column>		
			<display:column title="PKI" sortable="true">
				<c:forEach items="${modifCs.lstCnPki.cnPki}" var="pki">
					<c:out value="${pki}" />
					<br />
				</c:forEach>				
			</display:column>
			<display:column title="Certificats" sortable="true">
				<c:forEach items="${modifCs.lstCnCert.cnCert}" var="cert">
					<c:out value="${cert}" />
					<br />
				</c:forEach>
			</display:column>
			<display:column title="Durée VI" sortable="true">
				<c:out value="${modifCs.viduree}" />
			</display:column>			
			<display:column title="Vérif certificat" sortable="true">
				<c:if test="${modifCs.verifCnCert==true}">
					<c:out value="Oui" />
				</c:if>
				<c:if test="${modifCs.verifCnCert==false}">
					<c:out value="Non" />
				</c:if>
			</display:column>
