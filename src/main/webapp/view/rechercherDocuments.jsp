...
<body>
	...
	<form:form method="post" modelAttribute="rechercheForm"
		action="afficherDocuments.do">

		<div style="text-align: center; margin-top: 10px;" class="cadreForm">
			<label for="choixFrontal">Frontal sélectionné :</label>
			<form:select cssClass="inputSelect" id="choixFrontal" path="frontal"
				onchange="submit();">
				<form:options items="${listNomFrontaux}" />
			</form:select>
			</br>
			<label for="modeRechercheURLGED" class="inline">Désactiver le passage par le frontal</label>
			<form:checkbox id="modeRechercheURLGED" path="modeRechercheURLGED"  />
		</div>
		<table style="text-align: center; margin-top: 20px;">
			<thead>
				<tr>
					<td colspan="2">Détail du Frontal</td>
				</tr>
			</thead>
			<tr>
				<td>URL des services Web SaeService Frontal :</td>
				<td>${frontal.urlWsFrontal}</td>
			</tr>
			<tr>
				<td>Nom du Frontal</td>
				<td>${frontal.nom}</td>
			</tr>
		</table>
		...
		<p>
			Veuillez saisir la requête lucène (<a id="lienCodeRecherche"
				href="javascript:afficherCacherDiv('codeRecherche')"
				onclick="javascript:changerLien();">Voir la liste des
				métadonnées disponibles</a>) :
		</p>
		<p>
			<form:textarea path="requeteLucene" cols="105" rows="3"
				title="Requête Lucène" />
		</p>


		<div id="codeRecherche"
			style="overflow: scroll; height: 180px; display: none; padding-top: 10px">
			<display:table name="listeMetaRecherche" id="meta1"
		...
		<p>
			<a id="lienMetaAffichage"
				href="javascript:afficherCacherDiv('metaAffichage')"
				onclick="javascript:changerLienMetaAff();">Personnaliser les
				métadonnées à afficher en retour</a>
		</p>

		<div id="metaAffichage"
			style="overflow: scroll; height: 180px; display: none;">
		...
		<c:if test="${resRecherche != null}">
			<p>
				<c:out value="${resRecherche.nbResultat}" />
				document(s) trouvé(s)
			...
			<div style="overflow: scroll">
				<display:table name="resRecherche.donnees" id="docRow"
					requestURI="/afficherDocuments.do" pagesize="${nbLignesTableau}"
					style="width: 90%">
					<display:column title="Identifiant" sortable="true">
						<a href="lancerConsultDoc.do?identifiant=${docRow.uuid}"
							title="Consulter le document">${docRow.uuid}</a>
					</display:column>
