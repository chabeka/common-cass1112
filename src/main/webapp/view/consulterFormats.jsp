...
<form:form method="post" modelAttribute="formatListeForm" action="consulterFormats.do">
    ...
    <div class="cadreForm">
      ...
      <div><input type="submit" value="Rafraîchir" onclick="resetAction()" /></div>
...
<c:if test="${not empty listeFormats}">
   ...
   <display:table
      ...
      <c:if test="${formatRow.autoriseGED == true}">
      	<display:column title="Autorisé en GED" sortable="true">Oui</display:column>
      </c:if>
      <c:if test="${formatRow.autoriseGED == false}">
      	<display:column title="Autorisé en GED" sortable="true">Non</display:column>
      </c:if>
      ...
      <display:column property="convertisseur" title="Convertisseur" sortable="true"></display:column>
