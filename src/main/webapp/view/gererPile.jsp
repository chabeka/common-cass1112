...
<script>
	...
	function confirmerRelance(idJob) {
   		document.getElementById("action").value = "relancer"; 
   		document.forms['formulaireGestion'].elements['identifiantATraiter'].value = idJob;
		return confirm("Confirmer la relance du job : " + idJob + " ?");
	}
...
<form:form name="formulaireGestion" method="post"
	...
	<div style="padding-top: 10px"><a id="lienCriteres"
		href="javascript:afficherListeCriteres()"
		onclick="javascript:changerLien();">Afficher les critères de recherche</a>
	...
	<p class="double"><label for="parametres">Paramètre :</label> <form:input
		id="parametres" path="parameters" /></p>
	...
	<p style="text-align: right"><!-- Pour info, filter <=> rechercher sans relancer la requête en base -->
	<!-- <input type="submit" value="Filtrer" name="bouton_validation" onclick="javascript:formulaireGestion.action.value = 'filtrer'/> -->
	...
	<div style="padding-top: 15px">
		<display:table name="historique">
			<display:column property="date" title="Date" format="{0,date,dd/MM/yyyy HH:mm}"/>		
			<display:column property="trace" title="Trace du job ${identifiantATraiter}" />
		</display:table>
	</div>
	</c:if>
	...
	<div style="overflow: scroll; padding-top: 15px"><display:table
		...
		<display:column style="width: 10; align:center" title="">
			...
			<c:choose>
				<c:when test="${jobRow.resettable == 'true'}">
					<input title="Relancer le job" src="img/arrow_undo.png"
						type="image" value="Relancer"
						onclick="return confirmerRelance('${jobRow.idJob}')" />
				</c:when>
				<c:otherwise>
					<input
						style="cursor: default; filter: alpha(opacity =      40); opacity: 0.4;"
						title="Relancer le job" src="img/arrow_undo.png" type="image"
						value="Relancer"
						onclick="return confirmerRelance('${jobRow.idJob}')"
						disabled="disabled" />
				</c:otherwise>
		...
		<display:column style="width: 10; align:center" title="">
			<input title="Voir l'historique" src="img/calendar.png" type="image" value="Historique"
						onclick="return voirHistorique('${jobRow.idJob}')" />		
		</display:column>
		
		<display:column property="idJob" title="idJob"/>
		<display:column property="creationDate" title="creationDate" sortable="true" format="{0,date,dd/MM/yyyy HH:mm}" />
