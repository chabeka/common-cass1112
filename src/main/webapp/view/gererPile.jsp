				...
				<display:table name="listeJobAffiches" id="jobRow"
					...
					<c:forEach items="${gererPileTravauxForm.colonnesAffichees}"
						...
						<c:if test="${colonne == 'parameters'}">
							<display:column style="width:50px;" title="parameters" sortable="true">
								<c:choose>
									...
									<c:otherwise>
										<c:forEach var="parameterModel" items="${jobRow.jobParametersModel}">
											<div>
												...
												<c:set var="lengthParam" value="${fn:length(parameterModel.valueJobParam)}" />
												<c:set var="nbLoop" value="${fn:length(parameterModel.valueJobParam)/50}" />
												
												<c:if test="${lengthParam <= 50}">
													<c:out value="${parameterModel.valueJobParam}"/>
												</c:if>
												<c:if test="${lengthParam > 50}">
													<c:set var="valParamToList" value="" />
													<c:set var="newline" value="<%= \"\n\" %>" />
													<c:forEach var="counterLengthParam" begin="0" end="${nbLoop}">
												    	<c:if test="${lengthParam >= counterLengthParam*50 }">
															<c:set var="valParamToList" value="${valParamToList}${newline}
																${fn:substring(parameterModel.valueJobParam,counterLengthParam*50, (counterLengthParam+1)*50)}" />
												    	</c:if>
													</c:forEach>
														<c:out value="${valParamToList}"/>
												</c:if>
											</div>
										</c:forEach>
									</c:otherwise>
