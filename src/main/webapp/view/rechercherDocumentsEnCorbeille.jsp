...
<head>
<title>IHM Web Exploit - Recherche de documents dans la corbeille</title>
...
<script>
	...
	function cocher(aFaire) {
		var Chckbox = document.getElementsByName("shortCodeMeta");
...
<form:form method="post" modelAttribute="rechercheForm" action="afficherDocumentsEnCorbeille.do">
	...
	<div id="codeRecherche" style="overflow: scroll; height: 180px; display: none; padding-top: 10px">
		...
		requestURI="/rechercherDocumentsEnCorbeille.do" style="width: 90%;">
	...
	<div id="metaAffichage" style="overflow: scroll; height: 180px; display: none;">
		...
		<display:column style="text-align:center" title="<input type='checkbox' onClick='javascript:GereChkbox();'/>">	
			<form:checkbox path="shortCodeMeta" value="${meta2.shortCode}" />
		</display:column>
		<display:column property="code" title="Code" sortable="true" />
		<display:column property="libelle" title="Libelle" sortable="true" />
		<display:column property="indexee" title="Indexée" sortable="true" />
	...
	<c:if test="${resRecherche != null}">
		...
		<display:table name="resRecherche.donnees" id="docRow" requestURI="/afficherDocumentsEnCorbeille.do" pagesize="${nbLignesTableau}" style="width: 90%">
			<display:column title="Identifiant" sortable="true">
				<a href="lancerConsultDocEnCorbeille.do?identifiant=${docRow.uuid}" title="Consulter le document">${docRow.uuid}</a>
			</display:column>
