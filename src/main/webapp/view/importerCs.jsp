...
<script type="text/javascript">
   ...
   function importCS() {
      ...
      if (codeCsToImport.length == 0) {
         alert('Veuillez sélectionner au moins un contrat de service à importer !');
   		 ...
   		 var codesCsSession = listCodesCsSession.substr(1, listCodesCsSession.length -1);
         var listCodeCsSession = codesCsSession.split(",");
         for ( var j=0; j< codeCsToImport.length; j++) {
            if (listCodeCsSession.toString().indexOf(codeCsToImport[j].toString()) >= 0) {
               confirmImport = true;
      ...
      if (confirmImport) {
         if (confirm('Le(s) contrat(s) de service: ' + listCodeCsExistant
               + ' existe(nt) déjà ! ' + ' Voulez-vous vraiment le(s) écraser ? ')) {
   ...
   function exportCS(){
      var listCsToImport = document.getElementsByName("codeCsToImport");      
      // TODO
      document.forms['csForm'].elements['listCsToLoad'].value = document.getElementsByName("listeCs").value;
...
<body>
	...
	<form:form method="post" modelAttribute="csForm" name="csForm"
		...
		<div style="display: none;"> 
			<form:select cssClass="inputSelect" id="listCsSession" name="listCsSession"
				path="listCsSession">
		...
		<div style="display: none;"> 
			<form:select cssClass="inputSelect" id="listCsToLoad" name="listCsToLoad"
				path="listCsToLoad">
		...
		<div class="cadreForm">
			...
			<p style="text-align: right">
				<input type="button" id="exportButton" value="Exporter" onclick="exportCS()" />
				<input type="button" id="importButton" value="Importer" onclick="importCS()" />
			</p>
		</div>
		<div>
			<br />
