...
public class CRTL54_2IT extends AbstractTestBase {

   private static Logger logger = Logger.getLogger(CRTL54_2IT.class);
   ...
   public static void setUp() {
      logger.info("setUp " + CRTL54_2IT.class.getCanonicalName());
      connect();
      key = createBaseCategory("key", CategoryDataType.STRING, true, false, (short) 1, (short) 1,
            true);
      id = createBaseCategory("id", CategoryDataType.STRING, true, false, (short) 1, (short) 10,
            false);
      name = createBaseCategory("name", CategoryDataType.STRING, true, false, (short) 1,
            (short) 10, false);
   ...
   public static void tearDown() {
      ...
      disconnect();
      logger.info("tearDown " + CRTL54_2IT.class.getCanonicalName());
   }

   private static void storeDocuments(int nb, Base base, boolean virtual) {
      FileReference fileReference = null;
      if (virtual) {
         fileReference = serviceProvider.getStorageAdministrationService().createFileReference(
               base.getBaseId(), "pdf", TestUtils.getInputStream("doc1.pdf"));
      }
      for (int i = 0; i < nb; i++) {
         ...
         document.addCriterion(name, "name" + StringUtils.leftPad(String.valueOf(i), 3, "0"));
         if (virtual) {
            try {
               serviceProvider.getStoreService()
                     .storeVirtualDocument(document, fileReference, 1, 4);
            ...
            try {
               serviceProvider.getStoreService().storeDocument(document, "originalFilename", "pdf",
   ...
   @Test
   public void testSimpleRange() throws ExceededSearchLimitException, FrozenDocumentException,
   ...
   @Test
   public void testSimpleStar() throws ExceededSearchLimitException, FrozenDocumentException,
         SearchQueryParseException {

      // Check that document have been properly stored in base1 and base2.
      SearchResult search = serviceProvider.getSearchService().search(name.getName() + ":name*",
            200, base1);
      assertEquals(20, search.getTotalHits());
   }

   @Test
   public void testDeleteAll() throws TagControlException, ExceededSearchLimitException,
      ...
      search = serviceProvider.getSearchService().search(name.getName() + ":[name000 TO name019]",
            200, base2);
      assertEquals(20, search.getTotalHits());

      // Search all documents that share the same id value on every base
      search = serviceProvider.getSearchService()
            .multiBaseSearch(id.getName() + ":testDelete", 100);
