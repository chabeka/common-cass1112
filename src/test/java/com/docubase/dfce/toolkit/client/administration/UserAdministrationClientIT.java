...
public class UserAdministrationClientIT extends AbstractTestBase {
   ...
   public void before() throws ObjectAlreadyExistsException {
      ...
      serviceProvider.connect(AbstractTestBase.ADM_LOGIN, AbstractTestBase.ADM_PASSWORD,
            AbstractTestBase.SERVICE_URL);

      Set<UserPermission> permissions = new HashSet<UserPermission>();
      permissions.add(UserPermission.BASE_CREATE);
      GROUP_NAME = "groupName" + UUID.randomUUID();
      serviceProvider.getUserAdministrationService().createUserGroup(GROUP_NAME, permissions,
            baseNames);
   }

   @Test
   public void testCreateUser() throws ObjectAlreadyExistsException {
      User user = serviceProvider.getUserAdministrationService().createUser(userNameRandom,
            password, GROUP_NAME);

      assertNotNull(user);
      assertEquals(userNameRandom, user.getLogin());
      serviceProvider.connect(userNameRandom, password, AbstractTestBase.SERVICE_URL);
      assertTrue(serviceProvider.isSessionActive());
   }

   @Test
   public void testDisableUser() throws ObjectAlreadyExistsException {
      user = serviceProvider.getUserAdministrationService().loadUser(userNameRandom);
      if (user == null) {
         user = serviceProvider.getUserAdministrationService().createUser(userNameRandom, password,
               GROUP_NAME);
      }
      User disableUser = serviceProvider.getUserAdministrationService().disableUser(user);
   ...
   public void testEnableUser() throws ObjectAlreadyExistsException {
      ...
      User disableUser = serviceProvider.getUserAdministrationService().enableUser(user);
   ...
   public void testLoadUser() throws ObjectAlreadyExistsException {
      ...
      user = serviceProvider.getUserAdministrationService().createUser(userName, password,
            GROUP_NAME);
      User loadUser = serviceProvider.getUserAdministrationService().loadUser(userName);
   ...
   public void testUpdateUserPassword() throws ObjectAlreadyExistsException {
      ...
      serviceProvider.getUserAdministrationService().updateUserPassword(user, "newPassword");
      serviceProvider.disconnect();
      serviceProvider.connect(userNameRandom, "newPassword", AbstractTestBase.SERVICE_URL);
      assertTrue(serviceProvider.isSessionActive());
   }
}
