...
public class ExtractionFondsDocProdPourPgSqlTest {
   ...
   public void extraitFondsDoc() throws IOException {
      ...
      File rep = new File(repertoireSortie);
      if (!rep.exists()) {
         rep.mkdir();
      }
      ...
      try {
         ...
         if (!unFichierParMois) {
            File fichier = new File(repertoireSortie, "fonds_doc.csv");
   ...
   private Writer getWriter(boolean unFichierParMois, String repertoireSortie, String date, Map<String, Writer> writers) {
   ...
   private Writer getWriter(String repertoireSortie, String date, Map<String, Writer> writers) {
      
      String anneeMois = StringUtils.left(date, 6);
      
      Writer writer = writers.get(anneeMois);
      if (writer==null) {
         File fichier = new File(repertoireSortie, String.format("fonds_doc_%s.csv", anneeMois));
         try {
