...
public class MessageChannelPartitionHandler implements PartitionHandler {
   ...
   * <li>reply
   * channel that returns a list of {@link StepExecution} results</li>
   * </ul>
   ...
   *          the {@link MessagingOperations} to set
   */
  ...
  public List<?> aggregate(@Payloads final List<?> messages) {
    return messages;
  }
   ...
   */
  @Override
  public Collection<StepExecution> handle(final StepExecutionSplitter stepExecutionSplitter,
                                          final StepExecution masterStepExecution)
      throws Exception {

    final Set<StepExecution> split = stepExecutionSplitter.split(masterStepExecution, gridSize);
    int count = 0;
    final PollableChannel replyChannel = new QueueChannel();

    for (final StepExecution stepExecution : split) {
      final Message<StepExecutionRequest> request = createMessage(count++,
                                                                  split.size(),
    ...
    final Message<Collection<StepExecution>> message = (Message<Collection<StepExecution>>) messagingGateway.receive(replyChannel);
    if (logger.isDebugEnabled()) {
      logger.debug("Received replies: " + message);
    }
    final Collection<StepExecution> result = message.getPayload();
    return result;

  }

  private Message<StepExecutionRequest> createMessage(final int sequenceNumber, final int sequenceSize,
                                                      final StepExecutionRequest stepExecutionRequest, final PollableChannel replyChannel) {
