...
public class ChunkMessageItemWriterIntegrationTests {
  ...
  public void setUp() {
    ...
    final MessagingTemplate gateway = new MessagingTemplate();
  ...
  public void testVanillaIteration() throws Exception {
    ...
    final Step step = (Step) factory.getObject();

    final StepExecution stepExecution = getStepExecution(step);
  ...
  public void testSimulatedRestartWithBadMessagesFromAnotherJob() throws Exception {
    ...
    final String message = stepExecution.getExitStatus().getExitDescription();
  ...
  private GenericMessage<ChunkRequest> getSimpleMessage(final String string, final Long jobId) {
    final StepContribution stepContribution = new JobExecution(new JobInstance(0L, new JobParameters(), "job"), 1L)
                                                                                                                   .createStepExecution("step")
                                                                                                                   .createStepContribution();
    final ChunkRequest chunk = new ChunkRequest(0, StringUtils.commaDelimitedListToSet(string), jobId, stepContribution);
    final GenericMessage<ChunkRequest> message = new GenericMessage<>(chunk);
    return message;
  }
  ...
  public void testFailureInStepListener() throws Exception {
    ...
    final String exitDescription = stepExecution.getExitStatus().getExitDescription();
   ...
   */
  private void waitForResults(final int expected, final int maxWait) throws InterruptedException {
    int count = 0;
  ...
  private StepExecution getStepExecution(final Step step) throws JobExecutionAlreadyRunningException, JobRestartException,
      JobInstanceAlreadyCompleteException {
    final SimpleJob job = new SimpleJob();
    job.setName("job");
    final JobExecution jobExecution = jobRepository.createJobExecution(job.getName(),
                                                                       new JobParametersBuilder().addLong(
                                                                                                          "job.counter",
                                                                                                          jobCounter++)
                                                                                                 .toJobParameters());
