...
public class ConvertSommaireDocumentTransfertProcessor implements
   ...
   @Override
   public UntypedDocument process(JAXBElement<DocumentTypeMultiAction> itemTransfert)
         throws Exception {
      ...
      if (itemTransfert.getValue().getMetadonnees() != null) {
         if (itemTransfert.getValue().getMetadonnees().getMetadonnee() != null
               || !itemTransfert.getValue().getMetadonnees().getMetadonnee().isEmpty()) {
      ...
      untypedDoc.setBatchActionType(itemTransfert.getValue().getTypeAction().value());
