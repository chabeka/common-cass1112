 ...
 * 
 */
package fr.urssaf.image.sae.services.batch.transfert.support.resultats.impl;
 ...
 */
@Component
public class ResultatsFileEchecTransfertSupportImpl implements
ResultatsFileEchecTransfertSupport {
  ...
  private static final Logger LOGGER = LoggerFactory
      .getLogger(ResultatsFileEchecTransfertSupportImpl.class);
  ...
  private void ecrireFichierResultat(final XmlReader reader,
    ...
    if (Constantes.BATCH_MODE.TOUT_OU_RIEN.getModeNom().equals(
      ...
      if (listIntDocs != null && !listIntDocs.isEmpty()) {
        ...
        ecrireFichierResultatDocumentsIntegres(reader, isVirtual,
                                               erreur, listIntDocs);
      } else {
  ...
  private void endElementVirtuel(final String name) {
    if ("documentsVirtuels".equals(name)) {
      ...
      staxUtils.addEndTag(NON_INTEGRATED_VIRT_DOC, PX_SOMRES, NS_SOMRES);
   ...
   *         false sinon.
   */
  private boolean isDocumentDansListe(final ConcurrentLinkedQueue<?> listIntDocs,
                                      final IndexReference indexReference, final XMLEvent xmlEvent) {
    TraitementMasseIntegratedDocument document = null;
    if (listIntDocs != null && !listIntDocs.isEmpty()) {
      document = getDocumentInListByXmlEvent(xmlEvent, listIntDocs);
    }
   ...
   */
  private IndexReference addTagsNonIntegratedDocumentByTagName(
                                                               final String baliseName, final XMLEvent xmlEvent, final CaptureMasseErreur erreur,
  ...
  private void startTagDocsIntegres(final String name, final XmlReader reader,
    ...
    if (CHEMIN_FICHIER.equals(name)) {
      addIntegratedDocumentByTagName(CHEMIN_FICHIER, reader,
                                     listIntDocs);
    } else if (UUID_FICHIER.equals(name)) {
      addIntegratedDocumentByTagName(UUID_FICHIER, reader, listIntDocs);
    }
   ...
   *           Liste de documents
   */
  private void addIntegratedDocumentByTagName(final String tagName,
                                              final XmlReader reader, final ConcurrentLinkedQueue<?> listIntDocs) {
    final XMLEvent xmlEvent = reader.peek();
    final TraitementMasseIntegratedDocument document = getDocumentInListByXmlEvent(xmlEvent, listIntDocs);
    if (document != null) {
      addTagIntegratedDocumentByTagName(tagName, xmlEvent, document,
                                        reader);
    }
  }
  ...
  private void addNombrePages(final XmlReader reader)
      throws ResultatEchecLectureXMLException {
    copieXmlEventReaderParNom(
                              trouverXmlEventReaderParNom(reader, NOMBRE_PAGES),
                              NOMBRE_PAGES);
  }
  ...
  private void addNumeroPageDebut(final XmlReader reader)
      throws ResultatEchecLectureXMLException {
    copieXmlEventReaderParNom(
                              trouverXmlEventReaderParNom(reader, NUMERO_PAGE_DEBUT),
                              NUMERO_PAGE_DEBUT);
  }
   ...
   */
  private void ecrireEntete(final int nombreDocs, final int nombreDocsIntegres,
