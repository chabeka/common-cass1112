package fr.urssaf.image.sae.services.batch.suppression.support.traces;
 ...
 */
@Component
public class TracesServicesSuppressionMasseSupport {

  private static final Logger LOGGER = LoggerFactory
      .getLogger(TracesServicesSuppressionMasseSupport.class);

  @Autowired
  private DispatcheurService dispatcheurService;

  /**
   * Ecrit une trace d'échec de suppression de masse, le cas échéant
   * 
   * @param jobExecution
   *           le jobExecution de SpringBatch
   */
  @SuppressWarnings("PMD.AvoidCatchingThrowable")
  public final void traceEchecSuppressionMasse(final JobExecution jobExecution) {
    ...
    try {
      ...
      // Traces
      final String prefix = "traceEchecSuppressionMasse()";
      ...
      if (ExitStatus.COMPLETED.equals(jobExecution.getExitStatus())) {
        ...
        LOGGER
        .debug(
               "{} - On ne va pas tracer d'événement d'échec la suppression de masse",
               prefix);
        ...
        LOGGER
        .debug(
               "{} - On va tracer l'événement d'échec de la suppression de masse",
               prefix);
        ...
        // Code de l'événement
        traceToCreate.setCodeEvt(Constantes.TRACE_CODE_EVT_ECHEC_SM);

        // Contexte
        traceToCreate.setContexte(Constantes.CONTEXTE_SUPPRESSION_MASSE);
  ...
  private String buildStackTrace(final JobExecution jobExecution) {
    ...
    final ConcurrentLinkedQueue<Exception> exceptionsDocs = getSuppressionExceptions(jobExecution);
    nbEx = sizeCollection(exceptionsDocs);
    sBuilder.append(String.format("Exception(s) de suppression : %s%n",
                                  nbEx));
    if (nbEx > 0) {
  ...
  private ConcurrentLinkedQueue<Exception> getSuppressionExceptions(
    ...
    return getListeExceptions(jobExecution, Constantes.SUPPRESSION_EXCEPTION);
  }
