...
public class CheckBaseTest {
   ...
   public void init() throws Exception {
      ...
      cluster = HFactory.getOrCreateCluster("Docubase",
            new CassandraHostConfigurator("cer69imageint9.cer69.recouv:9160"));
   ...
   /**
    * Récupère la liste des catégories associées à la base "baseName", en se
    * basant sur la CF BasesReference et BaseCategoriesReference
    * 
    * @param baseName
    * @return
    * @throws Exception
    */
   private List<String> getCategoriesFromBasesReference(String baseName)
         throws Exception {

      List<String> categories = new ArrayList<String>();
      StringSerializer stringSerializer = StringSerializer.get();
      BytesArraySerializer bytesSerializer = BytesArraySerializer.get();
      CqlQuery<byte[], String, byte[]> cqlQuery = new CqlQuery<byte[], String, byte[]>(
            keyspace, bytesSerializer, stringSerializer, bytesSerializer);
      String query = "select * from BasesReference where Key ='"
            + ConvertHelper.stringToHex(baseName) + "'";
   ...
   /**
    * Récupère la liste des catégories associées à la base "baseName", en se
    * basant sur la CF BaseCategoriesReference
    * 
    * @param baseName
    * @return
    * @throws Exception
    */
   private List<String> getCategoriesFromBaseCategoriesReference(String baseName)
         throws Exception {
      ...
      CqlQuery<byte[], String, byte[]> cqlQuery = new CqlQuery<byte[], String, byte[]>(
            keyspace, bytesSerializer, stringSerializer, bytesSerializer);
      String query = "select * from BaseCategoriesReference where Key > '"
            + ConvertHelper.stringToHex(baseName) + "efbfbf'" + " and KEY < '"
            + ConvertHelper.stringToHex(baseName) + "ffff'";
      ...
      for (Row<byte[], String, byte[]> row : orderedRows) {
         ...
         if (!baseId.equals(baseName))
            throw new Exception("Base id inattendu : " + baseId);
         String categoryReference = getColumnValue(columns, "categoryReference");
         if (!categoryReference.equals(categorie))
            throw new Exception("categoryReference inattendu : "
                  + categoryReference);
    ...
    * @throws Exception
    */
   private String getColumnValue(List<HColumn<String, byte[]>> columns,
