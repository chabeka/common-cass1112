...
public class DescribeTest {
   ...
   public void init() throws Exception {
      ...
      final String servers = "hwi69intgnscas1.gidn.recouv:9160,hwi69intgnscas2.gidn.recouv:9160";
   ...
   private void describeKeyspace(final String nomKeyspace, final boolean avecCf) {

      final KeyspaceDefinition ksDef = cluster.describeKeyspace(nomKeyspace);
      describeKeyspace(ksDef, avecCf);
   }

   private void describeKeyspace(final KeyspaceDefinition ksDef, final boolean avecCf) {
      ...
      if (ksDef.getStrategyOptions().isEmpty()) {
         ...
         for (final Map.Entry<String, String> entry : ksDef.getStrategyOptions().entrySet()) {
   ...
   private void describeColumnFamily(final ColumnFamilyDefinition cfDef) {
      ...
      if (cfDef.getCompactionStrategyOptions().isEmpty()) {
         ...
         for (final Map.Entry<String, String> entry : cfDef.getCompactionStrategyOptions().entrySet()) {
   ...
   private void describeKeyspaces(final boolean avecCf) {
      final List<KeyspaceDefinition> ksDefs = cluster.describeKeyspaces();
      for (final KeyspaceDefinition ksDef : ksDefs) {
         describeKeyspace(ksDef, avecCf);
         System.out.println();
      }
   }
